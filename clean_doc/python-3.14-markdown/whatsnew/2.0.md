...
`\xHH` escape sequence can also be used, and octal escapes can be used for
...
* *encode\_func* is a function that takes a Unicode string, and returns a 2-tuple
...
* *decode\_func* is the opposite of *encode\_func*, taking an 8-bit string and
  ...
  was consumed.
* *stream\_reader* is a class that supports decoding input from a stream.
  *stream\_reader(file\_obj)* returns an object that supports the `read()`,
  `readline()`, and `readlines()` methods. These methods will all
  translate from the given encoding and return Unicode strings.
* *stream\_writer*, similarly, is a class that supports encoding output to a
  stream. *stream\_writer(file\_obj)* returns an object that supports the
...
which inserts the `str()` of its argument. For example, `'%r %s' % ('abc',
'abc')` returns a string containing `'abc' abc`.

Previously there was no way to implement a class that overrode Python’s built-in
`in` operator and implemented a custom version. `obj in seq` returns
...
the argument sequences. The difference between `zip()` and `map(None,
seq1, seq2)` is that `map()` pads the sequences with `None` if the
...
`sys` module. `sys.version_info` is a tuple `(major, minor, micro,
...
also works. `socket.connect_ex` and `socket.bind`
...
difficult, “bdist\_rpm” and “bdist\_wininst” commands have already been
