...
* The expression is evaluated and should result in an object called a “context
  manager”. The context manager must have `__enter__()` and
  `__exit__()` methods.
...
The decorator is called `contextmanager()`, and lets you write
...
```
importtime
frommultiprocessingimport Pool, Manager
...
* `RawIOBase` defines raw I/O operations: `read()`,
  ...
  There are also `readable()`, `writable()`,
  and `seekable()` methods for determining what operations a
...
* `BufferedIOBase` is an abstract base class that
  ...
  and adds a `raw` attribute holding the underlying
  raw object.
...
```
fromabcimport abstractproperty
...
...
* A new builtin, `next(iterator, [default])` returns the next item
  ...
  in bpo-2719.)
* Tuples now have `index()` and `count()` methods
  matching the list type’s `index()` and `count()` methods:
...
* Instance method objects have new attributes for the object and function
  ...
  `__self__`, and `im_func` is also available as
  `__func__`.
...
* A new data type in the `collections` module: `namedtuple(typename, fieldnames)`
...
* The `smtplib` module now supports SMTP over SSL thanks to the
  ...
  bpo-829951.)
* The `socket` module now supports TIPC (https://tipc.sourceforge.net/),
