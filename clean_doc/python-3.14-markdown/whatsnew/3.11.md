...
* **PEP 624**:
  Py\_UNICODE encoder APIs have been removed
* **PEP 670**:
...
* Added `object.__getstate__()`, which provides the default
  implementation of the `__getstate__()` method. `copy`ing
  and `pickle`ing instances of subclasses of builtin types
...
* Added keyword argument *all\_errors* to `asyncio.loop.create_connection()`
...
* Add optional parameter *dir\_fd* in `shutil.rmtree()`.
...
Python caches bytecode in the \_\_pycache\_\_
...
| --- | --- | --- | --- | --- |
| Binary operations | `x + x`  `x - x`  `x * x` | Binary add, multiply and subtract for common types such as `int`, `float` and `str` take custom fast paths for their underlying types. | 10% | Mark Shannon, Donghee Na, Brandt Bucher, Dennis Sweeney |
...
* The *population* parameter of `random.sample()` must be a sequence,
  and automatic conversion of `set`s to `list`s
...
* The `PyFrameObject` structure members have been removed from the
  ...
  Or use the pythoncapi\_compat project to get these two
