...
* In order to better support dynamic creation of stack traces,
  ...
  the `tb_next` attribute on
  tracebacks is now writable.
...
* The new `-X` `importtime` option or the
  ...
  (Contributed by Inada Naoki in bpo-31415.)
...
`compileall.compile_dir()` learned the new *invalidation\_mode* parameter,
...
`importlib.util.find_spec()` now raises `ModuleNotFoundError` instead of
...
The new `importlib.util.source_hash()` can be used to compute the hash of
...
The new *warn\_on\_full\_buffer* argument to the `signal.set_wakeup_fd()`
...
The `socket` module now exposes the socket.TCP\_CONGESTION (Linux 2.6.13), socket.TCP\_USER\_TIMEOUT (Linux 2.6.37), and socket.TCP\_NOTSENT\_LOWAT (Linux 3.12) constants.
...
The `subprocess.run()` function accepts the new *capture\_output*
...
`subprocess.Popen`. This means that *close\_fds* now defaults to
...
* in CPython debug builds, all warnings are now displayed
...
at the interactive prompt. See PEP 565: Show DeprecationWarning in \_\_main\_\_ for details.
...
The new import\_\_find\_\_load\_\_start and
import\_\_find\_\_load\_\_done static markers can be used
...
`datetime.date.fromordinal()` and `datetime.date.fromtimestamp()`
...
* The `ntpath.splitunc()` function was deprecated in
  Python 3.1, and has now been removed. Use `splitdrive()`
  instead.
