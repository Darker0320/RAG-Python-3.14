...
evaluated first, and the *true\_value* expression is evaluated only if the
condition was true. Similarly, the *false\_value* expression is only evaluated
...
https://unix.org/version2/whatsnew/lp64\_wp.html for a discussion – but the
...
* The `csv` module, which parses files in comma-separated value format,
  ...
  `csv.field_size_limit(new_limit)` function; omitting the *new\_limit*
...
* The largest change to the C API came from **PEP 353**, which modifies the
  ...
  int. See the earlier section PEP 353: Using ssize\_t as the index type for a discussion of this
  change.
...
* The built-in set types now have an official C API. Call `PySet_New()`
  and `PyFrozenSet_New()` to create a new set, `PySet_Add()` and
  `PySet_Discard()` to add and remove elements, and `PySet_Contains()`
  and `PySet_Size()` to examine the set’s state. (Contributed by Raymond
  Hettinger.)
...
* Two new macros can be used to indicate C functions that are local to the
  ...
  `Py_LOCAL_INLINE` does the same thing and also requests the
...
* C API: Many functions now use `Py_ssize_t` instead of int to
  ...
  earlier section PEP 353: Using ssize\_t as the index type for a discussion of this change.
