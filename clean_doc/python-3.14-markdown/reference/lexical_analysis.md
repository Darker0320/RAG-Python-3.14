...
tokens, generated by the *lexical analyzer* (also known as
the *tokenizer*).
...
(This is done even inside string literals.)
...
* non-ASCII characters. Valid names may only contain “letter-like” and
  “digit-like” characters; see Non-ASCII characters in names for details.
...
Non-ASCII characters in names for details.
...
In terms of lexical analysis, Python has string, bytes
...
See escape sequences below for a full list of such
...
String literals, except “f-strings” and
“t-strings”, are described by the
...
Changed in version 3.3: Support for name aliases
has been added.
...
See the Standard Library section on f-strings
...
Formatted string literals cannot be used as docstrings,
...
* There may be no whitespace in `FSTRING_START` (between
  the prefix and quote).
* Whitespace in `FSTRING_MIDDLE` is part of the literal
  string contents.
...
Python has complex number objects, but no complex
literals.
...
Generally, *operators* are used to combine expressions,
