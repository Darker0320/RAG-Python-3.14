...
modules. These are documented in The Python Standard Library. A few built-in modules
...
* `name`: A name refers to another rule.
  ...
  + `TOKEN`: An uppercase name refers to a token.
...
* `"text"`, `'text'`: Text in single or double quotes must match literally
  ...
  + `'@'`: A non-letter symbol in single quotes denotes an
    `OP` token, that is, a delimiter or
    operator.
...
of tokens generated by the lexical analysis.
