...
The simplest atoms are names or literals.
...
An identifier occurring as an atom is a name. See section Names (identifiers and keywords)
...
* Any name used as the name of a variable that is assigned or read or any
  ...
  The `__name__` attribute of nested functions, classes, and
...
See section String literal concatenation for details.
...
from the result, as if `iter()` was called on it.
...
result is `None`. Otherwise, if `send()` is used, then
...
:   The proposal to introduce the `yield_from` syntax,
...
:   Starts the execution of a generator function or resumes it at the last
    ...
    `yield_list` is returned to `__next__()`’s
...
:   Raises an exception at the point where the generator was paused,
    ...
    *type* constructor is called to get an instance. If *traceback*
    ...
    `__traceback__` attribute stored in *value* may
    be cleared.
...
:   Raises a `GeneratorExit` exception at the point where the generator
    ...
    exception), `close()` returns `None`. If the generator yields a
...
again, returning the value of `yield_list` to the
...
`asend()` is used, then the result will be the value passed in to that
method.
...
`aclose()` method to finalize the generator and ultimately detach it
from the event loop.
...
presumably calls `aclose()` and executes the coroutine.
...
`asyncio.Loop.shutdown_asyncgens` in Lib/asyncio/base\_events.py.
...
:   Returns an awaitable which when run starts to execute the asynchronous
    ...
    expression. The value of the `yield_list` of the
    yield expression is the value of the `StopIteration` exception raised by
...
Added in version 3.11: Expressions in *tuple\_slices* may be starred. See **PEP 646**.
...
operation can be overridden with the `__neg__()` special method.

The unary `+` (plus) operator yields its numeric argument unchanged; the
operation can be overridden with the `__pos__()` special method.
...
integers or one of them must be a custom object overriding `__and__()` or
`__rand__()` special methods.

The `^` operator yields the bitwise XOR (exclusive OR) of its arguments, which
must be integers or one of them must be a custom object overriding `__xor__()` or
`__rxor__()` special methods.

The `|` operator yields the bitwise (inclusive) OR of its arguments, which
must be integers or one of them must be a custom object overriding `__or__()` or
`__ror__()` special methods.
...
`__iter__()`, `x in y` is `True` if some value `z`, for which the
...
“walrus”) assigns an `expression` to an
`identifier`, while also returning the value of the
`expression`.
