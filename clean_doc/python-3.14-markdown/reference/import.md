...
subdirectory becomes a portion contributing to a namespace
...
The module’s spec is exposed as `module.__spec__`. Setting
...
The `__path__` attribute should be a (possibly empty)
...
`__path__`. `sys.path_hooks` (described below) are
...
value is stored in `sys.path_importer_cache`. Second, the value for the
...
portion, the path entry finder sets `submodule_search_locations` to
...
delete the default contents of `sys.meta_path`, replacing them
...
```
from.moduleYimport spam
from.moduleYimport spam as ham
from.import moduleY
from..subpackage1import moduleY
from..subpackage2.moduleZimport eggs
from..moduleAimport foo
```
