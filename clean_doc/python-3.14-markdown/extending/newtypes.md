...
structures (think: strings, tuples) which is where the `tp_itemsize` field
...
{
newdatatypeobject*self=(newdatatypeobject*)op;
returnPyUnicode_FromFormat("Repr-ified_newdatatype{{size:%d}}",
...
{
newdatatypeobject*self=(newdatatypeobject*)op;
returnPyUnicode_FromFormat("Stringified_newdatatype{{size:%d}}",
self->obj_UnderlyingDatatypePtr->size);
}
```
...
```
getattrfunctp_getattr;/* char * version */
setattrfunctp_setattr;
/* ... */
getattrofunctp_getattro;/* PyObject * version */
setattrofunctp_setattro;
```
...
```
typedefstructPyMethodDef{
constchar*ml_name;/* method name */
PyCFunctionml_meth;/* implementation function */
intml_flags;/* flags */
constchar*ml_doc;/* docstring */
}PyMethodDef;
```
...
`type` field should contain a type code like `Py_T_INT` or
`Py_T_DOUBLE`; the value will be used to determine how to
...
`Py_READONLY` to prevent Python code from setting it.
...
As with the `tp_methods` table, a sentinel entry with a `ml_name` value
of `NULL` is required.
...
returnPyLong_FromLong(self->data);
}
...
```
staticPyObject*
newdatatype_richcmp(PyObject*lhs,PyObject*rhs,intop)
{
...
switch(op){
casePy_LT:c=size1<size2;break;
casePy_LE:c=size1<=size2;break;
casePy_EQ:c=size1==size2;break;
casePy_NE:c=size1!=size2;break;
casePy_GT:c=size1>size2;break;
casePy_GE:c=size1>=size2;break;
}
result=c?Py_True:Py_False;
returnPy_NewRef(result);
}
```
...
`Py_TPFLAGS_MANAGED_WEAKREF` bit of the `tp_flags`
field. The legacy `tp_weaklistoffset` field should
be left as zero.

Concretely, here is how the statically declared type object would look:

```
staticPyTypeObjectTrivialType={
