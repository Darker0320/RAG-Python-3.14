...
call the `exit()` function or hold the `Ctrl` key down
...
after you call the `exit()` function or enter the `Ctrl`-`Z`
...
See How can I create a stand-alone binary from a Python script? for a list of tools that can be used to
make executables.
...
Python raises `IndentationError` or `TabError` if mixed tabs
and spaces are causing problems in leading whitespace.
You may also run the `tabnanny` module to check a directory tree
in batch mode.
