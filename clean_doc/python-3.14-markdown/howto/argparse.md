...
doesn’t support), and the very low level `getopt` (which specifically
...
* We’ve added the `add_argument()` method, which is what we use to specify
...
argument, `--` can be used to tell `parse_args()` that
...
into a `.po` file. For example, using Babel,
run this command:
...
installed using `gettext`, `argparse` will be able to display the
translated messages.
...
stored in the `argparse.Namespace`. This can be useful when you need to
