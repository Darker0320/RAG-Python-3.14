...
* Set the `PYTHON_DISABLE_REMOTE_DEBUG` environment variable to `1` before
...
This mechanism forms the basis of the `sys.remote_exec()` function, which
...
```
deffind_py_runtime_linux(pid: int) -> int:
...
```
deffind_py_runtime_macos(pid: int) -> int:
...
2. Identify the module corresponding to `python.exe` or
   `pythonXY.dll`, where `X` and `Y` are the major and minor
...
```
deffind_py_runtime_windows(pid: int) -> int:
    # Step 1: Try to find the Python executable in memory
...
```
defread_debug_offsets(pid: int, py_runtime_addr: int) -> DebugOffsets:
