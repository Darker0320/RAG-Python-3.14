...
Changed in version 3.14: The `QueueListener` can be started (and stopped) via the
...
process-management tool such as Supervisor.
Here is a Gist
...
The web application uses Gunicorn, which is a
...
merged into the `LogRecord` instance’s \_\_dict\_\_, allowing you to use
...
Since Python 3.7, the `contextvars` module has provided context-local storage
which works for both `threading` and `asyncio` processing needs. This type
...
Each `Handler` has its own chain of filters.
If you want to add contextual information to a `LogRecord` without leaking
it to other handlers, you can use a filter that returns
a new `LogRecord` instead of modifying it in-place, as shown in the following script:
...
file from your processes. The stdlib `FileHandler` and subclasses do
...
fromrandomimport choice, random
importtime
...
1. Attach a `Formatter` instance to your
...
* If you can’t refer to the callable directly in the configuration (e.g. if it
  ...
  in Access to external objects. For example, you could have used
  the text `'ext://__main__.MyFilter'` instead of `MyFilter` in the above
  example.
* As well as for filters, this technique can also be used to configure custom
  handlers and formatters. See User-defined objects for more
...
records buffered). These default to a `StreamHandler` which
...
`BufferingHandler`. In the following example, which you can
...
servers, the `SysLogHandler` functionality has not been
updated.
...
example, the `basicConfig()` call does this (using the
...
`basicConfig()`, but you can use a different formatter when you
configure logging.
...
frommymixinsimport MultilineMixin

logger = logging.getLogger(__name__)

classStreamHandler(MultilineMixin, logging.StreamHandler):
    pass
...
If, on the other hand, you are concerned about log injection, you can use a
...
* Forking a process, following which both parent and child have a reference to
  the same file. This might be through use of the `multiprocessing` module,
  for example.
...
Use the techniques outlined in Logging to a single file from multiple processes to circumvent such
issues.
...
the existing mechanisms for passing contextual
