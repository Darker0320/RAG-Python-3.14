...
This HOWTO article seeks to help you build a sturdy mental
model of how `asyncio` fundamentally works, helping you understand the
...
* This in-depth YouTube tutorial series on
...
of generators and
generator functions.
Recall, a generator function is a function that `yield`s, like this
one:
...
Roughly speaking, tasks are coroutines (not coroutine
...
loop.
`asyncio.run()` can help you detect such occurrences via the
...
`coroutine.send(arg)` is the method used to start or
resume a coroutine.
...
yield, as usual, pauses execution and returns control
to the caller.
In the example above, the `yield`, on line 3, is called by
`... = await rock` on line 11.
More broadly speaking, `await` calls the `__await__()` method of
the given object.
...
A future is an object meant to represent a
...
`asyncio.Task` subclasses `asyncio.Future` in order to gain
...
marked as done.
If `future.set_result()` (the method
...
```
async defasync_sleep(seconds: float):
...
```
classYieldToEventLoop:
    def__await__(self):
        yield

async def_sleep_watcher(future, time_to_wake):
    while True:
...
```
async defsimpler_async_sleep(seconds):
    time_to_wake = time.time() + seconds
    while True:
        if time.time() >= time_to_wake:
            return
