...
:   Equivalent to `exec_prefix`, but referring to the base Python installation.
    ...
    When running under Virtual Environments,
...
:   These objects contain the original values of `breakpointhook`,
    ...
    Added in version 3.7: \_\_breakpointhook\_\_

    Added in version 3.8: \_\_unraisablehook\_\_
...
:   A string giving the site-specific directory prefix where the platform-dependent
    ...
    **configure** script. Specifically, all configuration files (e.g. the
    ...
    `prefix` and `exec_prefix` are now set to the virtual
    environment prefix by the path initialization,
    instead of `site`. This means that `prefix` and
    `exec_prefix` always point to the virtual environment, even when
    `site` is disabled (`-S`).
...
:   The named tuple *flags* exposes the status of command line
    ...
    | flags.dont\_write\_bytecode | `-B` |
    | flags.no\_user\_site | `-s` |
    | flags.no\_site | `-S` |
    | flags.ignore\_environment | `-E` |
    | flags.verbose | `-v` |
    | flags.bytes\_warning | `-b` |
    | flags.quiet | `-q` |
    | flags.hash\_randomization | `-R` |
    | flags.dev\_mode | `-X dev` (Python Development Mode) |
    ...
    | flags.thread\_inherit\_context | `-X thread_inherit_context` and `PYTHON_THREAD_INHERIT_CONTEXT` |
    | flags.context\_aware\_warnings | `-X context_aware_warnings` and `PYTHON_CONTEXT_AWARE_WARNINGS` |
...
:   A named tuple holding information about the float type. It
    ...
    | --- | --- | --- |
    | float\_info.epsilon | `DBL_EPSILON` | difference between 1.0 and the least value greater than 1.0 that is representable as a float.  See also `math.ulp()`. |
    | float\_info.dig | `DBL_DIG` | The maximum number of decimal digits that can be faithfully represented in a float; see below. |
    | float\_info.mant\_dig | `DBL_MANT_DIG` | Float precision: the number of base-`radix` digits in the significand of a float. |
    | float\_info.max | `DBL_MAX` | The maximum representable positive finite float. |
    | float\_info.max\_exp | `DBL_MAX_EXP` | The maximum integer *e* such that `radix**(e-1)` is a representable finite float. |
    | float\_info.max\_10\_exp | `DBL_MAX_10_EXP` | The maximum integer *e* such that `10**e` is in the range of representable finite floats. |
    | float\_info.min | `DBL_MIN` | The minimum representable positive *normalized* float.  Use `math.ulp(0.0)` to get the smallest positive *denormalized* representable float. |
    | float\_info.min\_exp | `DBL_MIN_EXP` | The minimum integer *e* such that `radix**(e-1)` is a normalized float. |
    | float\_info.min\_10\_exp | `DBL_MIN_10_EXP` | The minimum integer *e* such that `10**e` is a normalized float. |
    | float\_info.radix | `FLT_RADIX` | The radix of exponent representation. |
...
sys.get\_int\_max\_str\_digits()
...
:   Return the reference count of the *object*. The count returned is generally one
    ...
    **CPython implementation detail:** Immortal objects with a large reference count can be
    identified via `_is_immortal()`.
...
:   Return a named tuple describing the Windows version
    ...
    *build*, *platform*, *service\_pack*, *service\_pack\_minor*,
    *service\_pack\_major*, *suite\_mask*, *product\_type* and
    *platform\_version*. *service\_pack* contains a string,
    *platform\_version* a 3-tuple and all other values are
    ...
    *product\_type* may be one of the following values:
    ...
    | Constant | Meaning |
    | --- | --- |
    | `1` (VER\_NT\_WORKSTATION) | The system is a workstation. |
    | `2` (VER\_NT\_DOMAIN\_CONTROLLER) | The system is a domain controller. |
    | `3` (VER\_NT\_SERVER) | The system is a server, but not a domain controller. |
    ...
    *platform\_version* returns the major version, minor version and
    ...
    *platform\_version* derives the version from kernel32.dll which can be of a different
    version than the OS version. Please use `platform` module for achieving accurate
    OS version.

    Availability: Windows.

    Changed in version 3.2: Changed to a named tuple and added *service\_pack\_minor*,
    *service\_pack\_major*, *suite\_mask*, and *product\_type*.

    Changed in version 3.6: Added *platform\_version*

sys.get\_asyncgen\_hooks()
:   Returns an *asyncgen\_hooks* object, which is similar to a
...
sys.get\_coroutine\_origin\_tracking\_depth()
...
:   A named tuple giving parameters of the numeric hash
    ...
    Changed in version 3.4: Added *algorithm*, *hash\_bits*, *seed\_bits*, and *cutoff*.
...
:   An object containing information about the implementation of the
    ...
    *cache\_tag* is the tag used by the import machinery in the filenames of
    ...
    *supports\_isolated\_interpreters* is a boolean value, whether
...
:   Utilities for observing just-in-time compilation.
    ...
    **CPython implementation detail:** JIT compilation is an *experimental implementation detail* of CPython.
    ...
    :   Return `True` if the current Python executable supports JIT compilation,
        ...
        `--enable-experimental-jit` option on all other platforms.
    ...
    :   Return `True` if JIT compilation is enabled for the current Python
        ...
        `PYTHON_JIT` environment variable to `0` (disabled) or `1`
        (enabled) at interpreter startup.

    \_jit.is\_active()
    :   Return `True` if the topmost Python frame is currently executing JIT
        code (implies `sys._jit.is_enabled()`), and `False` otherwise.
...
sys.\_is\_immortal(*op*)
:   Return `True` if the given object is immortal, `False`
    otherwise.
...
:   A string containing a platform identifier. Known values are:
    ...
    Changed in version 3.14: On FreeBSD, `sys.platform` doesn’t contain the major version anymore.
...
sys.set\_int\_max\_str\_digits(*maxdigits*)
...
sys.set\_asyncgen\_hooks(*[firstiter] [, finalizer]*)
...
sys.set\_coroutine\_origin\_tracking\_depth(*depth*)
...
sys.activate\_stack\_trampoline(*backend*, */*)
...
sys.deactivate\_stack\_trampoline()
...
sys.is\_stack\_trampoline\_active()
...
:   Executes *script*, a file containing Python code in the remote
    ...
    See Remote debugging attachment protocol for more information about the remote debugging
    mechanism.

    When the code is executed in the remote process, an
    auditing event `sys.remote_exec` is raised with
    the *pid* and the path to the script file.
    This event is raised in the process that called `sys.remote_exec()`.
