...
* UTC is Coordinated Universal Time and superseded Greenwich Mean Time or
...
* The time value as returned by `gmtime()`, `localtime()`, and
  ...
  Changed in version 3.3: The `struct_time` type was extended to provide
  the `tm_gmtoff` and `tm_zone`
  attributes when platform supports corresponding
  `struct tm` members.

  Changed in version 3.6: The `struct_time` attributes
...
:   Return the *clk\_id* of the thread-specific CPU-time clock for the specified *thread\_id*.
    ...
    See the man page for *pthread\_getcpuclockid(3)* for
    further information.
...
:   Return the resolution (precision) of the specified clock *clk\_id*. Refer to
    Clock ID Constants for a list of accepted values for *clk\_id*.
...
:   Set the time of the specified clock *clk\_id*. Currently,
    `CLOCK_REALTIME` is the only accepted value for *clk\_id*.
...
:   Like `gmtime()` but converts to local time. If *secs* is not provided or
    ...
    or `gmtime()` functions, and `OSError` on `localtime()` or
...
:   Return the value (in fractional seconds) of a performance counter, i.e. a
    ...
    **CPython implementation detail:** On CPython, use the same clock as `time.monotonic()` and is a
...
:   Convert a tuple or `struct_time` representing a time as returned by
    ...
    2. When used with the `strptime()` function, the `%p` directive only affects
    ...
    4. When used with the `strptime()` function, `%U` and `%W` are only used in
       calculations when the day of the week and the year are specified.

    Here is an example, a format for dates compatible with that specified in the
    **RFC 5322** Internet email standard. [1]
...
:   The type of the time value sequence returned by `gmtime()`,
    ...
    | 5 | tm\_sec | range [0, 61]; see Note (2) in `strftime()` |
    | 6 | tm\_wday | range [0, 6]; Monday is 0 |
    | 7 | tm\_yday | range [1, 366] |
    | 8 | tm\_isdst | 0, 1 or -1; see below |
    | N/A | tm\_zone | abbreviation of timezone name |
    | N/A | tm\_gmtoff | offset east of UTC in seconds |
...
time.CLOCK\_MONOTONIC\_RAW\_APPROX
:   Similar to `CLOCK_MONOTONIC_RAW`, but reads a value cached by
...
time.CLOCK\_UPTIME\_RAW\_APPROX
:   Like `CLOCK_UPTIME_RAW`, but the value is cached by the system
