...
`WSGIEnvironment` for a type alias that can be used
in type annotations.
...
:   Update *environ* with trivial defaults for testing purposes.
    ...
    Example usage (see also `demo_app()`
    for another example):
...
:   A concrete implementation of the `wsgiref.types.FileWrapper`
    ...
    are iterables. As the object is iterated over, the
...
:   Create a `WSGIServer` instance. *server\_address* should be a
...
:   Create an HTTP handler for the given *request* (i.e. a socket), *client\_address*
    ...
    get\_environ()
    :   Return a `WSGIEnvironment` dictionary for a
        request. The default
...
:   Similar to `BaseCGIHandler`, but designed for use with HTTP origin
    ...
    `__init__()`, `get_stdin()`,
    `get_stderr()`, `add_cgi_vars()`,
    `_write()`, and `_flush()` methods to
...
:   This is an abstract base class for running WSGI applications. Each instance
    ...
    get\_stdin()
    :   Return an object compatible with `InputStream`
        suitable for use as the `wsgi.input` of the
        request currently being processed.

    get\_stderr()
    :   Return an object compatible with `ErrorStream`
    ...
    :   This method is a WSGI application to generate an error page for the user. It is
        ...
        the *start\_response* callable should follow the `StartResponse`
        protocol.
    ...
    :   A `wsgi.file_wrapper` factory, compatible with
        `wsgiref.types.FileWrapper`, or `None`. The default value
        of this attribute is the `wsgiref.util.FileWrapper` class.
...
:   A `typing.Protocol` describing **start\_response()**
    callables (**PEP 3333**).
...
:   A `typing.Protocol` describing a **file wrapper**.
    See `wsgiref.util.FileWrapper` for a concrete implementation of this
    protocol.
