...
:   Returns a new instance of the `StreamHandler` class. If *stream* is
    ...
    :   If a formatter is specified, it is used to format the record. The record
        is then written to the stream followed by `terminator`. If exception information
...
:   Returns a new instance of the `FileHandler` class. The specified file is
    ...
    *errors* is specified, it’s used to determine how encoding errors are handled.
    ...
    :   Outputs the record to the file.
        ...
        mode is ‘w’, the record will not be emitted (see bpo-42378).
...
*ST\_INO* is not supported under Windows; `stat()` always returns zero
for this value.
...
:   The parameters are as for `FileHandler`. The attributes are:
    ...
    :   If this attribute is set to a callable, the `rotation_filename()`
        ...
        For example, `RotatingFileHandler` expects to have a set of log files
        whose names contain successive integers, so that rotation works as expected,
        and `TimedRotatingFileHandler` deletes old log files (based on the
        ...
        overrides the `getFilesToDelete()` method to
...
*class*logging.handlers.SysLogHandler(*address=('localhost', SYSLOG\_UDP\_PORT)*, *facility=LOG\_USER*, *socktype=socket.SOCK\_DGRAM*, *timeout=None*)
...
:   Initializes the handler with a buffer of the specified capacity. Here,
    *capacity* means the number of logging records buffered.
    ...
    emit(*record*)
    :   Append the record to the buffer. If `shouldFlush()` returns true,
...
:   Returns a new instance of the `MemoryHandler` class. The instance is
    ...
    flush()
    :   For a `MemoryHandler` instance, flushing means just sending the buffered
...
:   Returns a new instance of the `QueueHandler` class. The instance is
    ...
    :   Prepares a record for queuing. The object returned by this
        ...
        message (obtained by calling the handler’s `format()` method), and
        ...
        that a handler on the `QueueListener` side won’t have the
    ...
    :   When created via configuration using `dictConfig()`, this
        attribute will contain a `QueueListener` instance for use with this
...
:   Returns a new instance of the `QueueListener` class. The instance is
    ...
    `with`. When entering the context, the listener is started. When
    ...
    :   Starts the listener.
        ...
        Changed in version 3.14: Raises `RuntimeError` if called and the listener is already
        running.
