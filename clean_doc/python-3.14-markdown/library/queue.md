...
FIFO queue type, `SimpleQueue`, whose
...
:   Exception raised when `put()` or `get()` is called on
    a `Queue` object which has been shut down.
...
:   Put a `Queue` instance into a shutdown mode.
    ...
    Future calls to `put()` raise `ShutDown`.
