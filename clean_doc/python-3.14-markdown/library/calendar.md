...
:   Creates a `Calendar` object. *firstweekday* is an integer specifying the
    first day of the week. `MONDAY` is `0` (the default), `SUNDAY` is `6`.
...
:   This class can be used to generate HTML calendars.
    ...
    ```
    classCustomHTMLCal(calendar.HTMLCalendar):
...
:   A subclass of `ValueError` and `IndexError`,
    ...
    `ValueError`. New code should avoid catching
    `IndexError`.
...
```
python-mcalendar[-h][-LLOCALE][-eENCODING][-t{text,html}]
[-wWIDTH][-lLINES][-sSPACING][-mMONTHS][-cCSS]
[-fFIRST_WEEKDAY][year][month]
```

For example, to print a calendar for the year 2000:

```
$ python-mcalendar2000
                                  2000
...
:   The encoding to use for output.
    `--encoding` is required if `--locale` is set.
...
month
:   The month of the specified `year` to print the calendar for.
