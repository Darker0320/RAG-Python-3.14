...
`set.copy()`), and of sequences (such as lists or bytearrays) by making
a slice of the entire sequence (`sequence[:]`).
However, these methods and slicing can create an instance of the base type
when copying an instance of a subclass, whereas `copy.copy()` normally
...
special methods `__copy__()` and `__deepcopy__()`.
