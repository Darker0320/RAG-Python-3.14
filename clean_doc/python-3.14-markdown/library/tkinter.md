...
Tk
:   Tk is a Tcl package implemented in C
    that adds custom commands to create and manipulate GUI widgets. Each
    `Tk` object embeds its own Tcl interpreter instance with Tk loaded into
...
:   Themed Tk (Ttk) is a newer family of Tk widgets that provide a much better
    ...
    bindings are provided in a separate module, `tkinter.ttk`.
...
The `grid()` method is used to specify the relative layout (position)
...
`configure()` method, which returns a dictionary containing a variety
...
`Tk.mainloop`) and responds to events.
...
or can be queried at runtime by calling the `config()` method without
arguments, or by calling the `keys()` method on that widget.
...
`BooleanVar`.
To read the current value of such a variable, call the `get()`
method on it, and to change its value you call the `set()` method.
...
:   is a string that denotes the target kind of event. Physical events use the
    ...
    *bind(3tk)* man page, and page 201 of John Ousterhout’s book,
...
:   The `Misc` class is a mix-in inherited by `Tk` and, through
    ...
    :   Interpret the string *s* as a Tcl boolean and return the corresponding
        ...
        `'true'` and `'false'`.
        Raise `ValueError` if *s* is not a valid boolean.

    getdouble(*s*)
    :   Interpret the string *s* as a Tcl floating-point number and return it as
        a `float`.
        Raise `ValueError` if *s* is not a valid number.
    ...
    :   Interpret the string *s* as a Tcl integer and return it as an
        `int`.
        Raise `ValueError` if *s* is not a valid integer.
    ...
    :   Set the Tcl global variable named *name* to *value*.
        ...
        variables.
        In most code you will instead use a `Variable` subclass such as
        `StringVar` or `IntVar`, which wraps a Tcl variable and
    ...
    :   Return the names of all images that currently exist in the Tcl
        ...
        This is overridden by the `Text` widget,
    ...
    :   Return the bounding box, in pixels, of a region of the grid laid out in
        ...
        except on `Canvas`, `Listbox`, `Spinbox`,
        `Text`, `ttk.Entry` and
        `ttk.Treeview`,
    ...
    :   Query or set the properties of the row (or rows) *index* of the grid
        ...
        *index* is interpreted as for `grid_columnconfigure()`, and the
    ...
    :   Bind the event pattern *sequence* on this widget to the callable *func*.
        ...
        *sequence*; otherwise it replaces them.
        ...
        passed to `unbind()` to remove the binding without leaking the
    ...
    :   Like `bind()`, but bind *func* to the binding tag *className* rather
    ...
    :   Like `bind()`, but bind *func* to the special binding tag `'all'`,
    ...
    :   Remove bindings for the event pattern *sequence* on this widget.
        ...
        returned from a previous call to `bind()`) is removed, and its
    ...
    :   Schedule the callable *func* to be called after *ms* milliseconds, with
        *args* and *kw* passed to it as positional and keyword arguments.
        Return an identifier that can be passed to `after_cancel()` to cancel
        the call.
    ...
    :   Cancel a callback previously scheduled with `after()` or
        ...
        *id* must be an identifier returned by one of those methods; passing a
        value that is not such an identifier raises `ValueError`.
    ...
    :   If *id* is omitted, return a tuple of the identifiers of all callbacks
        currently scheduled with `after()` and `after_idle()` for this
        interpreter.
        ...
        *script* refers to the function to be called and *type* is either
        `'idle'` or `'timer'`.
        A `TclError` is raised if *id* does not exist.
    ...
    :   Wait until the Tcl variable *name* is modified, continuing to process
        ...
        *name* is usually a `Variable` instance, such as an
        `IntVar` or `StringVar`.
    ...
    :   Return the widget that currently has the keyboard focus in the
        ...
        Use `focus_displayof()` to work correctly with several displays.
    ...
    :   Return the previous widget before this one in the keyboard traversal
        ...
        See `tk_focusNext()` for how the order is defined.
    ...
    :   Set a global grab on this widget.
        A global grab is like the local grab set by `grab_set()`, but it
    ...
    :   Clear the X selection, so that no window owns it anymore.
        ...
        This is overridden by the `Entry`, `Listbox` and
        `Spinbox` widgets,
    ...
    :   Append *string* to the Tk clipboard and claim ownership of the clipboard
        ...
        `clipboard_clear()`; all appends should be completed before returning
        ...
        The contents can be retrieved with `clipboard_get()` or
        `selection_get()`.
    ...
    :   Make this widget appear busy.
        ...
        Normally `update()` should be called immediately afterwards to ensure
    ...
    :   Query or modify the configuration options of the busy window.
        ...
        For example, if a `Frame` widget named `frame` is to be made
        ...
        `busy_configure()`, `busy_config()` and `tk_busy_config()`
    ...
    :   Return the textual name for the atom whose integer identifier is *id*.
        This is the inverse of `winfo_atom()`.
    ...
    :   Return the top-of-hierarchy window containing the widget.
        In standard Tk this is always a `Toplevel` widget.
    ...
    :   Return a list describing the visuals available for the widget’s screen.
        Each item consists of a visual class (see `winfo_visual()`) followed
        by an integer depth.
...
:   The `Wm` mixin provides access to the window manager, allowing an
    ...
    :   Query or set platform-specific attributes of the window.
        ...
        *class*, *stylemask*, *tabbingid* and *tabbingmode* attributes.
        ...
        `wm_attributes()` is an alias of `attributes()`.
    ...
    :   Store *name*, which should be the name of the host on which the
        ...
        `wm_client()` is an alias of `client()`.
    ...
    :   Manipulate the `WM_COLORMAP_WINDOWS` property, which tells the window
        ...
        property.
        `wm_colormapwindows()` is an alias of `colormapwindows()`.
    ...
    :   Store *value* in the window’s `WM_COMMAND` property for use by the
        ...
        `wm_command()` is an alias of `command()`.
    ...
    :   Display the window in normal (non-iconified) form by mapping it.
        ...
        `wm_deiconify()` is an alias of `deiconify()`.
    ...
    :   Set or query the bitmap used by the window manager for the window’s icon.
        *bitmap* names a bitmap in one of the standard forms accepted by Tk; an
        ...
        `wm_iconbitmap()` is an alias of `iconbitmap()`.
    ...
    :   Iconify the window.
        ...
        `wm_iconify()` is an alias of `iconify()`.
    ...
    :   Set or query the bitmap used as a mask for the icon (see
        ...
        string.
        `wm_iconmask()` is an alias of `iconmask()`.
    ...
    :   Set the titlebar icon for the window from one or more `PhotoImage`
        ...
        `wm_iconphoto()` is an alias of `iconphoto()`.
    ...
    :   Make *widget* a stand-alone top-level window, decorated by the window
        ...
        Only `Frame`, `LabelFrame` and `Toplevel` widgets
        may be used (the `tkinter.ttk` versions are **not** accepted);
        passing any other widget type raises an error.
        `wm_manage()` is an alias of `manage()`.
    ...
    :   Set or query the maximum permissible dimensions of the window, in pixels
        ...
        *height*.
        ...
        `wm_maxsize()` is an alias of `maxsize()`.
    ...
    :   Set or query the minimum permissible dimensions of the window, in pixels
        ...
        *height*.
        ...
        `wm_minsize()` is an alias of `minsize()`.
    ...
    :   Set or query the override-redirect flag for the window.
        ...
        `wm_overrideredirect()` is an alias of `overrideredirect()`.
    ...
    :   Set or query the source of the window’s current position.
        *who* is either `'program'` or `'user'` and indicates whether the
        ...
        has been set.
        Tk automatically sets the source to `'user'` when `geometry()` is
    ...
    :   Register *func* as the handler for the window manager protocol *name*, an
        ...
        `wm_protocol()` is an alias of `protocol()`.
    ...
    :   Set or query the source of the window’s current size.
        *who* is either `'program'` or `'user'` and indicates whether the
        ...
        With no argument, return the current source, or an empty string if none
        has been set.
        `wm_sizefrom()` is an alias of `sizefrom()`.
    ...
    :   Set or query the state of the window.
        ...
        `iconwindow()`); the `'icon'` state cannot be set.
        `wm_state()` is an alias of `state()`.
    ...
    :   Withdraw the window from the screen, unmapping it and causing the window
        ...
        `wm_withdraw()` is an alias of `withdraw()`.
...
:   Geometry manager that arranges widgets by packing them against the sides of
    ...
    *pack* geometry manager.
    ...
    :   Unmap the widget and remove it from the packing order, forgetting its
        packing options.
        It can be packed again later with `pack_configure()`.
        `forget()` is an alias of `pack_forget()`,
        except on `PanedWindow`,
        `ttk.Notebook` and
        `ttk.PanedWindow`,
    ...
    pack\_propagate(*flag*)
    :   Same as `Misc.pack_propagate()`, treating this widget as a container:
        enable or disable geometry propagation.
        `propagate()` is an alias of `pack_propagate()`.

    pack\_slaves()
    :   Same as `Misc.pack_slaves()`: return the list of widgets packed in
        this widget.
        `slaves()` is an alias of `pack_slaves()`.
...
:   Geometry manager that places widgets at explicit positions and sizes within
    ...
    place\_slaves()
    :   Same as `Misc.place_slaves()`: return the list of widgets placed in
        this widget.
...
:   Geometry manager that arranges widgets in a two-dimensional grid of rows and
    ...
    grid\_columnconfigure(*index*, *cnf={}*, *\*\*kw*)
    :   Same as `Misc.grid_columnconfigure()`: query or set the options (such
        as *weight*, *minsize*, *pad* and *uniform*) of a grid column.
        `columnconfigure()` is an alias of `grid_columnconfigure()`.

    grid\_rowconfigure(*index*, *cnf={}*, *\*\*kw*)
    :   Same as `Misc.grid_rowconfigure()`: query or set the options of a
        grid row.
        `rowconfigure()` is an alias of `grid_rowconfigure()`.

    grid\_location(*x*, *y*)
    :   Same as `Misc.grid_location()`: return the `(column, row)` of the
        cell that covers the pixel at *x*, *y*.
        `location()` is an alias of `grid_location()`.

    grid\_size()
    :   Same as `Misc.grid_size()`: return a `(columns, rows)` tuple giving
    ...
    grid\_slaves(*row=None*, *column=None*)
    :   Same as `Misc.grid_slaves()`: return the widgets managed in the grid,
...
:   Mix-in providing the horizontal-scrolling interface shared by widgets such
    ...
    :   Query or change the horizontal position of the view.
        ...
        are usually generated by a scrollbar; `xview_moveto()` and
        `xview_scroll()` provide a more convenient interface.
...
:   Mix-in providing the vertical-scrolling interface shared by widgets such as
    `Canvas`, `Listbox` and `Text`.
    A widget’s `yview()` method is registered as the *command* of a vertical
    `Scrollbar`.
    ...
    :   Query or change the vertical position of the view.
        ...
        usually generated by a scrollbar; `yview_moveto()` and
        `yview_scroll()` provide a more convenient interface.
...
:   Construct a toplevel Tk widget, which is usually the main window of an
    ...
    *use*
    ...
    :   The Tk application object created by instantiating `Tk`. This
        ...
        the same instance of `Tk` has the same value for its `tk`
        attribute.

    master
    :   The widget object that contains this widget.
        For `Tk`, the `master` is `None` because it is the
        main window.
    ...
    :   Finish loading and initializing the Tk subsystem.
        ...
        example through `Tcl()`); it is called automatically when *useTk* is
        true.
    ...
    :   Report a callback exception.
        ...
        *exc*, *val* and *tb* are the exception type, value and traceback as
        ...
        The default implementation prints a traceback to `sys.stderr`.
...
:   A `Canvas` widget implements structured graphics.
    ...
    view can be scrolled horizontally and vertically with `xview()`
    and `yview()`.
    ...
    :   Return the current value of the configuration option *option* for the
        ...
        This is like `cget()` but applies to an individual item.

    itemconfigure(*tagOrId*, *cnf=None*, *\*\*kw*)
    :   Query or modify the configuration options of the items given by
        *tagOrId*.
        This mirrors `configure()`, except that it applies to
    ...
    :   Return a tuple of the ids of all items selected by the search
        ...
        The search specification has any of the forms accepted by `addtag()`.
    ...
    :   Return a tuple containing the id of the item closest to the point (*x*,
        *y*).
        *halo* and *start* are interpreted as for `addtag_closest()`.
    ...
    :   Move all items given by *tagOrId* to a new position in the display list
        ...
        `lower()` is an alias of `tag_lower()`.

        Note

        On a `Canvas`, `tkraise()`/`lift()` and `lower()`
        restack canvas items,
        shadowing the inherited `Misc.tkraise()`/`Misc.lift()` and
        `Misc.lower()` methods that restack the widget itself,
    ...
    :   Bind the callback *func* to the event *sequence* for all items given by
        *tagOrId*, so that *func* is invoked whenever that event occurs for one
        of the items.
        This is like `Widget.bind` but operates on canvas items
    ...
    :   Remove for all items given by *tagOrId* the binding for the event
        *sequence*.
        ...
        `tag_bind()`) is unbound and deregistered.
    ...
    :   Return a 4-tuple `(x1, y1, x2, y2)` giving an approximate bounding box,
        ...
        *tagOrIds*.
        ...
        use `grid_bbox()` for the grid bounding box.
    ...
    :   With *tagOrId*, set the keyboard focus for the canvas to the first item
        ...
        use `focus_set()` to focus the widget itself.
    ...
    :   Set the selection to the characters of *tagOrId* between the selection
        ...
        The anchor point is the one set by the most recent `select_adjust()`
        or `select_from()` call.
    ...
    :   Scroll the canvas by *gain* times the difference between *x*, *y* and the
        coordinates passed to the last `scan_mark()` call.
    ...
    :   Generate a PostScript (Encapsulated PostScript, version 3.0)
        ...
        it is usually necessary either to call `update()` first or to
    ...
    *validate* and *validatecommand* (which together let a callback accept or
    ...
    :   Locate the end of the selection nearest to the character given by
        ...
        *index* and the most recent anchor point, inclusive.
        `select_adjust()` is an alias of `selection_adjust()`.
    ...
    :   Clear the selection if it is currently in this widget.
        ...
        This shadows the inherited `Misc.selection_clear()`,
    ...
    :   Return `True` if there are characters selected in the entry, `False`
        otherwise.
        `select_present()` is an alias of `selection_present()`.
    ...
    :   Set the selection to include the characters starting with the one indexed
        ...
        `select_range()` is an alias of `selection_range()`.
    ...
    :   Set the selection between the anchor point and *index*: if *index* is
        ...
        The anchor point is the one set by the most recent `selection_from()`
        or `selection_adjust()` call.
        If there is no selection in the entry, a new one is created using the
        most recent anchor point.
        `select_to()` is an alias of `selection_to()`.
    ...
    :   Compute the difference between *x* and the *x* given to the last
        `scan_mark()` call, and adjust the view left or right by 10 times
        that difference.
...
:   A `Listbox` widget displays a list of single-line text items, one
    ...
    :   Return the total number of items in the listbox.
        ...
        use `grid_size()` for the grid size.
    ...
    :   Return `True` if the item given by *index* is currently selected,
        `False` otherwise.
        `select_includes()` is an alias of `selection_includes()`.
    ...
    :   Select all of the items in the range from *first* to *last* inclusive,
        ...
        `select_set()` is an alias of `selection_set()`.
    ...
    :   Query or modify the configuration options of the item given by *index*.
        This mirrors `configure()`, except that it applies to an
...
:   A `Menu` widget displays a column of entries, each of which may be a
    ...
    :   Same as `add()`, except that the new entry is inserted just before
    ...
    :   Query or modify the configuration options of the entry given by *index*.
        ...
        The supported options are those accepted by `add()` for the entry’s
        type.
...
:   A `Menubutton` widget displays a textual string, bitmap or image and
    posts an associated `Menu`, given by its *menu* option, when the user
    presses it.
    Like a `Label` it can show *text*, a *textvariable*, or an *image*,
...
:   A `Message` widget displays a non-interactive textual string, given
    ...
    Unlike a `Label`, it breaks the string into multiple lines in order
...
:   A `PanedWindow` is a geometry-manager widget that arranges any
    ...
    :   Add *child* to the panedwindow as a new pane, placed after any existing
        ...
        they may be any of the options accepted by `paneconfigure()`.
    ...
    :   Remove the pane containing *child* from the panedwindow.
        ...
        use `pack_forget()`, `grid_forget()` or
        `place_forget()` to remove the widget itself from its manager.
    ...
    :   Return the current value of the management option *option* for the pane
        containing *child*.
        *option* may be any value allowed by `paneconfigure()`.
    ...
    :   Query or modify the management options of the pane containing the widget
        *tagOrId*.
        ...
        `paneconfig()` is an alias of `paneconfigure()`.
    ...
    :   Query or change the position of the sashes in the panedwindow.
        ...
        methods `sash_coord()`, `sash_mark()` and `sash_place()`
...
:   A `Scrollbar` widget displays a slider and two arrows that let the
    ...
    *xscrollcommand* or *yscrollcommand* option to the scrollbar’s `set()`
    ...
    :   Return the current scrollbar settings as a tuple `(first, last)` of two
        ...
        currently visible, as last passed to `set()`.
...
:   A `Spinbox` widget is an `Entry`-like widget with a pair of
    ...
    *wrap* option controls whether stepping past either end of the range wraps
    ...
    :   A thin wrapper around the Tk `scan` widget subcommand, used to
        ...
        that mark.
        The `scan_mark()` and `scan_dragto()` methods wrap the two forms.

    scan\_mark(*x*)
    :   Record *x* and the current view in the spinbox window, for use with a
        later `scan_dragto()` call.
    ...
    :   A thin wrapper around the Tk `selection` widget subcommand, used to
        ...
        `selection_present()`, `selection_range()` and
        `selection_to()` methods wrap these forms.
    ...
    :   Locate the end of the selection nearest to the character given by *index*
        ...
        going beyond *index*).
        The other end becomes the anchor point for future `selection_to()`
        calls.
...
:   A `Text` widget displays and edits multi-line text.
    ...
    :   Replace the range of characters from *index1* up to but not including
        *index2* with *chars*.
        This is equivalent to a `delete()` followed by an `insert()` at
    ...
    :   Bind the event *sequence* on characters tagged with *tagName* to the
        ...
        *sequence*, otherwise it replaces them.
        Works like `bind()` and returns the identifier of the new
        binding.

    tag\_unbind(*tagName*, *sequence*, *funcid=None*)
    :   Remove the bindings of the event *sequence* on characters tagged with
        *tagName*.
        If *funcid* is given, only that binding (as returned by `tag_bind()`)
    ...
    :   Query or modify the configuration options of the embedded image at
        *index*, like `configure()` but applied to that image.
    ...
    :   Return a tuple of the names of all images embedded in the widget.
        ...
        This shadows the inherited `Misc.image_names()`,
    ...
    :   Query or modify the configuration options of the embedded window at
        *index*, like `configure()` but applied to that window.
    ...
    :   Undo the most recent edit action, that is, all the inserts and deletes
        ...
        the redo stack.
        Raises `TclError` if the undo stack is empty.
    ...
    :   Reapply the most recently undone edit action, provided no further edits
        ...
        Raises `TclError` if the redo stack is empty.
        Has no effect unless the *undo* option is true.
        Since Tk 9.0, returns a tuple of indices delimiting the ranges of text
        that were changed.
    ...
    :   Search for *pattern* starting at *index* and return the index of the
        ...
        *nocase* makes the match case-insensitive; and *elide* causes hidden text
        ...
        If *count* is a `Variable`, the number of index positions in the
...
:   The base class for the Tk variable wrappers.
    ...
    *name* is the name of the variable in the Tcl interpreter; if omitted, a
    ...
    :   Remove a trace callback from the variable.
        *mode* must match the *mode* that was passed to `trace_add()`, and
    ...
    :   Register *callback* to be called when the variable is accessed according
        ...
        *mode* is one of the strings `'r'`, `'w'` or `'u'`, for read, write
        ...
        `trace()` is an alias of `trace_variable()`.

        Deprecated since version 3.6: Use `trace_add()` instead. This method wraps a Tcl feature that
    ...
    :   Remove the trace callback named *cbname* registered for *mode* with
        ...
        Deprecated since version 3.6: Use `trace_remove()` instead. This method wraps a Tcl feature
        that was removed in Tcl 9.0.

    trace\_vinfo()
    :   Return a list of `(mode, cbname)` pairs for all traces set on the
        variable with `trace_variable()`.

        Deprecated since version 3.6: Use `trace_info()` instead. This method wraps a Tcl feature that
        was removed in Tcl 9.0.

*class*tkinter.StringVar(*master=None*, *value=None*, *name=None*)
:   A `Variable` subclass that holds a string.
...
:   A `Variable` subclass that holds an integer.
...
:   A `Variable` subclass that holds a boolean.
    The default value is `False`.

    get()
    :   Return the value of the variable as a `bool`.
        Raise a `ValueError` if the value cannot be interpreted as a
        boolean.
...
:   Base class for Tk images.
    ...
    *image* option; deleting all references to the `Image` object
    ...
    :   Modify one or more configuration options of the image.
        The valid options depend on the image type; see `PhotoImage` and
        `BitmapImage`.
        `config()` is an alias of `configure()`.
...
:   The `Tcl()` function is a factory function which creates an object much
    ...
    created (and the Tk subsystem initialized) by calling its `loadtk()`
    method.
...
:   Run the Tk main event loop on the default root window until all windows are
    destroyed.
    Equivalent to calling `Misc.mainloop()` on the default root.
...
:   Convert the Tcl boolean string *s* (one of `'1'`, `'true'`, `'yes'`,
    ...
    `bool`.
    Raise `TclError` for an invalid value.

tkinter.getdouble(*s*)
:   Convert *s* to a floating-point number.
    This is the built-in `float`.

tkinter.getint(*s*)
:   Convert *s* to an integer.
    This is the built-in `int`.
...
:   Compass directions (`'n'`, `'s'`, `'e'`, `'w'` and the diagonals and
    ...
    *sticky* options and by methods such as `Misc.grid_anchor()`.
...
tkinter.SEL\_LAST
:   Symbolic indices used by the `Text`, `Entry`, `Listbox`
    and `Canvas` widgets, such as `'insert'` (the insertion cursor),
...
tkinter.SEPARATOR
:   Menu entry types, used as the *itemType* argument of `Menu.add()` and
    `Menu.insert()`.
