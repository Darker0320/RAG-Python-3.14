...
Language, https://toml.io). This module does not
...
:   Load TOML from a `str` object. Return a `dict`. Convert TOML
    ...
    *parse\_float* argument has the same meaning as in `load()`.
...
:   Subclass of `ValueError` with the following additional attributes:
    ...
    Added the `msg`, `doc`, `pos`, `lineno` and `colno` attributes.
