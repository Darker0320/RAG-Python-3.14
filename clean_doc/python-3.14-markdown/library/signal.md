...
* If the handler raises an exception, it will be raised “out of thin air” in
  the main thread. See the note below for a
  discussion.
...
`enums` (`Signals`, `Handlers` and `Sigmasks` respectively).
...
:   `enum.IntEnum` collection of SIG\* constants and the CTRL\_\* constants.
...
:   `enum.IntEnum` collection of the constants `SIG_BLOCK`, `SIG_UNBLOCK` and `SIG_SETMASK`.

    Availability: Unix.

    See the man page *sigprocmask(2)* and
    *pthread\_sigmask(3)* for further information.
...
:   Stack fault on coprocessor. The Linux kernel does not raise this signal: it
    ...
    the man page *signal(7)* for further information.
...
:   Raised to signal an error from the underlying `setitimer()` or
    ...
    Added in version 3.3: This error used to be a subtype of `IOError`, which is now an
    alias of `OSError`.
