...
This includes the encodings specified in
**RFC 4648** (Base64, Base32 and Base16), the Base85 encoding specified in PDF 2.0, and non-standard variants
...
:   Encode the bytes-like object *s* using Base64 and return the encoded
    ...
    May assert or raise a `ValueError` if the length of *altchars* is not 2. Raises a
    `TypeError` if *altchars* is not a bytes-like object.
...
:   Similar to `b32encode()` but uses the Extended Hex Alphabet, as defined in
    **RFC 4648**.
