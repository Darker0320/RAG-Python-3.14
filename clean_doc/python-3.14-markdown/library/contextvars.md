...
state. The `ContextVar` class is used to declare
and work with *Context Variables*. The `copy_context()`
function and the `Context` class should be used to
...
:   This class is used to declare a new Context Variable, e.g.:
    ...
    **Important:** Context Variables should be created at the top module
    ...
    :   Call to set a new value for the context variable in the current
        ...
        to avoid calling `ContextVar.reset()` manually:
...
:   *Token* objects are returned by the `ContextVar.set()` method.
    ...
    *set*. A single token cannot reset a context variable more than once.
    ...
    var
    :   A read-only property. Points to the `ContextVar` object
        that created the token.

    old\_value
    :   A read-only property. Set to the value the variable had before
        the `ContextVar.set()` method call that created the token.
        It points to `Token.MISSING` if the variable was not set
        before the call.
...
:   A mapping of `ContextVars` to their values.
    ...
    *Exiting* from the current context, which can be done by returning from the
    callback passed to the `run()` method, restores the current
