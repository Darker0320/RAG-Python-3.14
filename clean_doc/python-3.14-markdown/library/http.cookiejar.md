...
:   Instances of `FileCookieJar` raise this exception on failure to load
    ...
    Changed in version 3.3: `LoadError` used to be a subtype of `IOError`, which is now an
    alias of `OSError`.
...
:   Constructor arguments should be passed as keyword arguments only.
    *blocked\_domains* is a sequence of domain names that we never accept cookies
    from, nor return cookies to. *allowed\_domains* if not `None`, this is a
...
https://curl.se/rfc/cookie\_spec.html
...
:   Add correct *Cookie* header to *request*.
    ...
    If policy allows (that is, the `rfc2965` and
    `hide_cookie2` attributes of
    ...
    `type`, `unverifiable`
    and `origin_req_host` as documented by
    `urllib.request`.
...
:   Extract cookies from HTTP *response* and store them in the `CookieJar`,
    ...
    *Set-Cookie2* headers in the *response* argument, and store cookies
    ...
    `info()` method, which returns an
    `email.message.Message` instance.

    The *request* object (usually a `urllib.request.Request` instance)
    must support the method `get_full_url()` and
    the attributes `host`,
    `unverifiable`
    and `origin_req_host`,
...
:   Discard all session cookies.
    ...
    anyway, unless you ask otherwise by passing a true *ignore\_discard* argument.
...
FileCookieJar.load(*filename=None*, *ignore\_discard=False*, *ignore\_expires=False*)
...
argument and methods for *allowed\_domains*). If you set an allowlist, you can
...
```
importos,http.cookiejar,urllib.request
...
fromhttp.cookiejarimport CookieJar, DefaultCookiePolicy
