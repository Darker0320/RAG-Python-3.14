...
As of Python 3.13, free-threaded builds
...
:   Handle uncaught exception raised by `Thread.run()`.
    ...
    * *thread*: Thread which raised the exception, can be `None`.
    ...
    If *exc\_type* is `SystemExit`, the exception is silently ignored.
...
Note that subclasses can define \_\_slots\_\_, but they are not
...
daemonic, and cannot be joined. They are never deleted,
...
:   This constructor should always be called with keyword arguments. Arguments
    ...
    *context* is the `Context` value to use when starting
    ...
    caller of `start()`. If false, they will start with an empty
    context. To explicitly start with an empty context, pass a new instance of
    `Context()`. To explicitly start with a copy of the
    current context, pass the value from `copy_context()`. The
    ...
    :   Start the thread’s activity.
        ...
        `threading.Thread.name`. The name can be truncated depending on the
    ...
    :   Wait until the thread terminates. This blocks the calling thread until
        ...
        of Python finalization `join()`
        raises a `PythonFinalizationError`.
    ...
    :   The Thread ID (`TID`) of this thread, as assigned by the OS (kernel).
        ...
        been started. See the `get_native_id()` function.
...
Threads call a lock’s `acquire()` method to lock it,
and its `release()` method to unlock it.

Note

Reentrant locks support the context management protocol,
so it is recommended to use `with` instead of manually calling
`acquire()` and `release()`
to handle acquiring and releasing the lock for a block of code.

RLock’s `acquire()`/`release()` call pairs may be nested,
unlike Lock’s `acquire()`/`release()`. Only the final
`release()` (the `release()` of the outermost pair) resets
the lock to an unlocked state and allows another thread blocked in
`acquire()` to proceed.

`acquire()`/`release()` must be used in pairs: each acquire
...
:   This class implements reentrant lock objects. A reentrant lock must be
    ...
    :   Acquire a lock, blocking or non-blocking.
        ...
        If called multiple times, failing to call `release()` as many times
