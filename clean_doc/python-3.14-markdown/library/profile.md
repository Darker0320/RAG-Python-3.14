...
:   This class is normally only used if more precise control over profiling is
    ...
    The `Profile` class can also be used as a context manager (supported
    only in `cProfile` module. see Context Manager Types):
    ...
    :   Create a `Stats` object based on the current
        ...
        multi-level sorting, as in `Stats.sort_stats`.
