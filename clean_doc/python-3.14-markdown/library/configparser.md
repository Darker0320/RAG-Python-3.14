...
:   The main configuration parser. When *defaults* is given, it is initialized
    ...
    `DuplicateOptionError`. When *empty\_lines\_in\_values* is `False`
    ...
    *empty\_lines\_in\_values*, *default\_section* and *interpolation* were
    ...
    Changed in version 3.8: The default *dict\_type* is `dict`, since it now preserves
    insertion order.

    Changed in version 3.13: Raise a `MultilineContinuationError` when *allow\_no\_value* is
    ...
    :   Write a representation of the configuration to the specified file
        ...
        *space\_around\_delimiters* is true, delimiters between
        ...
        be accurately parsed by a future `read()` call from this parser.
...
:   Exception raised when attempting to use the
    `UNNAMED_SECTION` without enabling it.
