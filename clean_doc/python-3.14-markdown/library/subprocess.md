...
:   Subclass of `SubprocessError`, raised when a timeout expires
    ...
    :   Output of the child process if it was captured by `run()` or
        `check_output()`. Otherwise, `None`. This is always
        `bytes` when any output was captured regardless of the
    ...
    :   Stderr output of the child process if it was captured by `run()`.
        Otherwise, `None`. This is always `bytes` when stderr output
...
:   Execute a child program in a new process. On POSIX, the class uses
    ...
    *stdin*, *stdout* and *stderr* specify the executed program’s standard input,
    ...
    code using *preexec\_fn* to call `os.setsid()` or `os.setpgid()` in the child.
    ...
    Changed in version 3.2: The default for *close\_fds* was changed from `False` to
    what is described above.

    Changed in version 3.7: On Windows the default for *close\_fds* was changed from `False` to
    ...
    *close\_fds* to be `True`. (POSIX only)
...
:   Wait for child process to terminate. Set and return
    ...
    sleeps). Use the `asyncio` module for an asynchronous wait: see
...
:   Interact with process: Send data to stdin. Read data from stdout and stderr,
    ...
    `returncode` attribute. The optional *input* argument should be
    ...
    After a call to `communicate()` raises `TimeoutExpired`, do
    not call `wait()`. Use an additional `communicate()`
    call to finish handling pipes and populate the `returncode`
    attribute.
...
:   Sends the signal *signal* to the child.
    ...
    On Windows, SIGTERM is an alias for `terminate()`. CTRL\_C\_EVENT and
...
:   A `STARTUPINFO.dwFlags` parameter to specify that the
...
:   A `STARTUPINFO.dwFlags` parameter to specify that the mouse
