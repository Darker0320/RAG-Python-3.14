...
*class*mmap.mmap(*fileno*, *length*, *tagname=None*, *access=ACCESS\_DEFAULT*, *offset=0*)
    ...
    *offset* may be specified as a non-negative integer offset. mmap references
    ...
    :   Send advice *option* to the kernel about the memory region beginning at
        *start* and extending *length* bytes. *option* must be one of the
        MADV\_\* constants available on the system. If
...
mmap.MADV\_MERGEABLE

mmap.MADV\_UNMERGEABLE

mmap.MADV\_SOFT\_OFFLINE

mmap.MADV\_HUGEPAGE

mmap.MADV\_NOHUGEPAGE
...
mmap.MADV\_FREE\_REUSABLE

mmap.MADV\_FREE\_REUSE
...
mmap.MAP\_RESILIENT\_CODESIGN

mmap.MAP\_RESILIENT\_MEDIA

mmap.MAP\_STACK

mmap.MAP\_TPRO

mmap.MAP\_TRANSLATED\_ALLOW\_EXECUTE
