...
Changed in version 3.10: **PEP 644** has been implemented. The ssl module requires OpenSSL 1.1.1
or newer.
...
ssl.create\_default\_context(*purpose=Purpose.SERVER\_AUTH*, *\**, *cafile=None*, *capath=None*, *cadata=None*)
:   Return a new `SSLContext` object with default settings for
    ...
    *cadata* is given) or uses `SSLContext.load_default_certs()` to load
    ...
    Changed in version 3.10: The context now uses `PROTOCOL_TLS_CLIENT` or
    `PROTOCOL_TLS_SERVER` protocol instead of generic
    `PROTOCOL_TLS`.

    Changed in version 3.13: The context now uses `VERIFY_X509_PARTIAL_CHAIN` and
...
ssl.cert\_time\_to\_seconds(*cert\_time*)
...
ssl.get\_server\_certificate(*addr*, *ssl\_version=PROTOCOL\_TLS\_CLIENT*, *ca\_certs=None*[, *timeout*])
:   Given the address `addr` of an SSL-protected server, as a (*hostname*,
    *port-number*) pair, fetches the server’s certificate, and returns it as a
    ...
    Changed in version 3.5: The default *ssl\_version* is changed from `PROTOCOL_SSLv3` to
    `PROTOCOL_TLS` for maximum compatibility with modern servers.

    Changed in version 3.10: The *timeout* parameter was added.

ssl.DER\_cert\_to\_PEM\_cert(*der\_cert\_bytes*)
:   Given a certificate as a DER-encoded blob of bytes, returns a PEM-encoded
    string version of the same certificate.

ssl.PEM\_cert\_to\_DER\_cert(*pem\_cert\_string*)
:   Given a certificate as an ASCII PEM string, returns a DER-encoded sequence of
    bytes for that same certificate.

ssl.get\_default\_verify\_paths()
...
ssl.enum\_certificates(*store\_name*)
...
:   `enum.IntEnum` collection of CERT\_\* constants.
...
:   Selects the highest protocol version that both the client and server support.
    ...
    favor of `PROTOCOL_TLS_CLIENT` and `PROTOCOL_TLS_SERVER`.
...
:   Auto-negotiate the highest protocol version that both the client and
    ...
    protocol enables `CERT_REQUIRED` and
...
:   Selects SSL version 3 as the channel encryption protocol.
    ...
    protocol `PROTOCOL_TLS_SERVER` or `PROTOCOL_TLS_CLIENT`
    with `SSLContext.minimum_version` and
...
ssl.OP\_LEGACY\_SERVER\_CONNECT
...
:   `enum.IntEnum` collection of ALERT\_DESCRIPTION\_\* constants.
...
:   `enum.IntEnum` collection of SSL\_ERROR\_\* constants.
...
TLSVersion.MINIMUM\_SUPPORTED

TLSVersion.MAXIMUM\_SUPPORTED
...
:   SSL 3.0 to TLS 1.3.
    ...
    Deprecated since version 3.10: All `TLSVersion` members except `TLSVersion.TLSv1_2` and
...
SSLSocket.do\_handshake(*block=False*)
:   Perform the SSL setup handshake.

    If *block* is true and the timeout obtained by `gettimeout()`
...
SSLSocket.getpeercert(*binary\_form=False*)
...
SSLSocket.get\_verified\_chain()
...
SSLSocket.get\_unverified\_chain()
...
SSLSocket.get\_channel\_binding(*cb\_type='tls-unique'*)
...
SSLSocket.selected\_alpn\_protocol()
...
SSLSocket.selected\_npn\_protocol()
...
:   Create a new SSL context. You may pass *protocol* which must be one
    ...
    Deprecated since version 3.10: `SSLContext` without protocol argument is deprecated. The
    context class will either require `PROTOCOL_TLS_CLIENT` or
    ...
    `PROTOCOL_TLS`, `PROTOCOL_TLS_CLIENT`, and
    `PROTOCOL_TLS_SERVER` use TLS 1.2 as minimum TLS version.
...
SSLContext.load\_verify\_locations(*cafile=None*, *capath=None*, *cadata=None*)
...
SSLContext.get\_ca\_certs(*binary\_form=False*)
...
SSLContext.set\_alpn\_protocols(*alpn\_protocols*)
...
SSLContext.set\_npn\_protocols(*npn\_protocols*)
:   Specify which protocols the socket should advertise during the SSL/TLS
...
:   Register a callback function that will be called after the TLS Client Hello
    ...
    The `SSLSocket.getpeercert()`, `SSLSocket.get_verified_chain()`,
    `SSLSocket.get_unverified_chain()` `SSLSocket.cipher()`
    and `SSLSocket.compression()` methods require that
...
SSLContext.set\_servername\_callback(*server\_name\_callback*)
:   This is a legacy API retained for backwards compatibility. When possible,
    you should use `sni_callback` instead. The given *server\_name\_callback*
...
SSLContext.load\_dh\_params(*dhfile*, */*)
...
SSLContext.set\_ecdh\_curve(*curve\_name*, */*)
...
SSLContext.wrap\_socket(*sock*, *server\_side=False*, *do\_handshake\_on\_connect=True*, *suppress\_ragged\_eofs=True*, *server\_hostname=None*, *session=None*)
:   Wrap an existing Python socket *sock* and return an instance of
    ...
    *sock* must be a `SOCK_STREAM` socket; other
    ...
    raise a `ValueError` if *server\_side* is true.
    ...
    *session*, see `session`.
    ...
    Changed in version 3.7: The method returns an instance of `SSLContext.sslsocket_class`
    instead of hard-coded `SSLSocket`.

SSLContext.sslsocket\_class
:   The return type of `SSLContext.wrap_socket()`, defaults to
    `SSLSocket`. The attribute can be assigned to on instances of
    `SSLContext` in order to return a custom subclass of
    `SSLSocket`.

    Added in version 3.7.

SSLContext.wrap\_bio(*incoming*, *outgoing*, *server\_side=False*, *server\_hostname=None*, *session=None*)
:   Wrap the BIO objects *incoming* and *outgoing* and return an instance of
    ...
    Changed in version 3.7: The method returns an instance of `SSLContext.sslobject_class`
    instead of hard-coded `SSLObject`.

SSLContext.sslobject\_class
...
:   Whether to match the peer cert’s hostname in
    ...
    `CERT_REQUIRED`, and you must pass *server\_hostname* to
...
:   Control the number of TLS 1.3 session tickets of a
    `PROTOCOL_TLS_SERVER` context. The setting has no impact on TLS
...
:   An integer representing the set of SSL options enabled on this context.
    ...
    Python 3.7. Use `SSLContext.minimum_version` and
    `SSLContext.maximum_version` instead.
...
SSLContext.set\_psk\_client\_callback(*callback*)
...
SSLContext.set\_psk\_server\_callback(*callback*, *identity\_hint=None*)
