...
* Strings (`str`) and `bytes`.
  Bytes-like objects like `bytearray` are
  marshalled as `bytes`.
* Containers: `tuple`, `list`, `set`, `frozenset`,
  and (since `version` 5), `slice`.
...
* `code` objects, if *allow\_code* is true. See note above about
  version dependence.
...
:   Write the value on the open file. The value must be a supported type. The
    ...
    Code objects are only supported if *allow\_code* is true.
