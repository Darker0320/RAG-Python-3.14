...
:   The first version requires that *numerator* and *denominator* are instances
    ...
    usual issues with binary point (see Floating-Point Arithmetic: Issues and Limitations), the
    ...
    Changed in version 3.9: The `math.gcd()` function is now used to normalize the *numerator*
    and *denominator*. `math.gcd()` always returns an `int` type.
    Previously, the GCD type depended on *numerator* and *denominator*.

    Changed in version 3.11: Underscores are now permitted when creating a `Fraction` instance
    from a string, following **PEP 515** rules.

    Changed in version 3.11: `Fraction` implements `__int__` now to satisfy
    ...
    Changed in version 3.12: `Fraction` instances now support float-style formatting, with
    presentation types `"e"`, `"E"`, `"f"`, `"F"`, `"g"`, `"G"`
    and `"%""`.

    Changed in version 3.13: Formatting of `Fraction` instances without a presentation type
    now supports fill, alignment, sign handling, minimum width and grouping.

    Changed in version 3.14: The `Fraction` constructor now accepts any objects with the
    ...
    *classmethod*from\_float(*f*)
    :   Alternative constructor which only accepts instances of
        `float` or `numbers.Integral`. Beware that
    ...
    *classmethod*from\_number(*number*)
    :   Alternative constructor which only accepts instances of
        `numbers.Integral`, `numbers.Rational`,
        `float` or `decimal.Decimal`, and objects with
    ...
    :   Provides support for formatting of `Fraction` instances via the
        ...
        `float` type in the Format specification mini-language section.
