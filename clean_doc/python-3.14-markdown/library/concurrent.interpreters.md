...
By default, most objects are copied with `pickle` when they are
...
concurrent.interpreters.list\_all()
:   Return a `list` of `Interpreter` objects,
    one for each existing interpreter.

concurrent.interpreters.get\_current()
:   Return an `Interpreter` object for the currently running
    interpreter.

concurrent.interpreters.get\_main()
:   Return an `Interpreter` object for the main interpreter.
    This is the interpreter the runtime created to run the REPL
    or the script given at the command-line. It is usually the only one.

concurrent.interpreters.create()
:   Initialize a new (idle) Python interpreter
    and return a `Interpreter` object for it.

concurrent.interpreters.create\_queue()
...
:   This exception, a subclass of `TypeError`, is raised when
...
```
fromconcurrentimport interpreters
