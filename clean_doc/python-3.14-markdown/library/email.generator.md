...
As with the `email.parser` module, you aren’t limited to the functionality
...
assuming the same non-transforming `policy` is used for both. That
is, parsing the serialized byte stream via the
`BytesParser` class and then regenerating the serialized
byte stream using `BytesGenerator` should produce output identical to
the input [1]. (On the other hand, using the generator on an
`EmailMessage` constructed by program may result in
changes to the `EmailMessage` object as defaults are
filled in.)
...
As a convenience, `EmailMessage` provides the methods
`as_bytes()` and `bytes(aMessage)` (a.k.a.
`__bytes__()`), which simplify the generation of
...
*Content-Transfer\_Encoding*. Using the terminology of the email
RFCs, you can think of this as `Generator` serializing to an I/O stream
that is not “8 bit clean”. In other words, most applications will want
to be using `BytesGenerator`, and not `Generator`.

*class*email.generator.Generator(*outfp*, *mangle\_from\_=None*, *maxheaderlen=None*, *\**, *policy=None*)
:   Return a `Generator` object that will write any message provided
    to the `flatten()` method, or any text provided to the `write()`
...
`__str__()`), which simplify the generation of
