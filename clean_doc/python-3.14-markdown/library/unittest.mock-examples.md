...
Since Python 3.8, `AsyncMock` and `MagicMock` have support to mock
Asynchronous Context Managers through `__aenter__` and `__aexit__`.
...
`mock_open()` is used to patch `open()` method. `side_effect`
...
... deftest():
...     frompackage.moduleimport attribute
...
... deftest():
...     frompackage.moduleimport ClassName
...
For some tests, you may want to mock out a call to `datetime.date.today()`
to return a known date, but don’t want to prevent the code under test from
creating new date objects. Unfortunately `datetime.date` is written in C,
so you cannot just monkey-patch out the static `datetime.date.today()` method.
...
repetition. Instead, you can use `patch()` (in all its
...
instead. The `patch()` decorator makes it so simple to patch out methods
...
>>> fromunittest.mockimport Mock, patch, DEFAULT
...
When the `__getitem__()` and `__setitem__()` methods
...
...    frompackage.moduleimport fooble
...    fooble()
...
...
(hamcrest.library.integration.match\_equality).