...
:   name
    ...
    :   The list of handlers directly attached to this logger instance.
        ...
        the `addHandler()` and `removeHandler()` methods, which use locks to ensure
    ...
    :   Logs a message with level `DEBUG` on this logger. The *msg* is the
        ...
        *exc\_info*, *stack\_info*, *stacklevel* and *extra*.
        ...
        dictionary which is used to populate the `__dict__` of the
        `LogRecord`
...
:   \_\_init\_\_(*level=NOTSET*)
    ...
    :   Sets the formatter for this handler to *fmt*.
        The *fmt* argument must be a `Formatter` instance or `None`.
...
:   Responsible for converting a `LogRecord` to an output string
    ...
    :   * **fmt** (*str*) – A format string in the given *style* for
        ...
        * **style** (*str*) – Can be one of `'%'`, `'{'` or `'$'` and determines
          ...
          printf-style String Formatting (`%`), `str.format()` (`{`)
        ...
        * **validate** (*bool*) – If `True` (the default), incorrect or mismatched
...
:   Contains all the information pertinent to the event being logged.
    ...
    :   * **name** (*str*) – The name of the logger used to log the event
        ...
        * **exc\_info** (*tuple**[**type**[**BaseException**]**,* *BaseException**,* *types.TracebackType**]* *|* *None*) – An exception tuple with the current exception information,
...
| exc\_text | You shouldn’t need to format this yourself. | Exception information formatted as a string. This is set when `Formatter.format()` is invoked, or `None` if no exception has occurred. |
...
:   Logs a message with level `INFO` on the root logger. The arguments and behavior
    are otherwise the same as for `debug()`.

logging.warning(*msg*, *\*args*, *\*\*kwargs*)
:   Logs a message with level `WARNING` on the root logger. The arguments and behavior
    are otherwise the same as for `debug()`.
...
:   Logs a message with level `CRITICAL` on the root logger. The arguments and behavior
    are otherwise the same as for `debug()`.
...
:   Set a callable which is used to create a `LogRecord`.
    ...
    Added in version 3.2: This function has been provided, along with `getLogRecordFactory()`, to
