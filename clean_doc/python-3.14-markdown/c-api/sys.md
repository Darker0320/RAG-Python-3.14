...
intPy\_FdIsInteractive(FILE\*fp, constchar\*filename)
...
intPyOS\_CheckStack()
:   *Part of the Stable ABI on platforms with USE\_STACKCHECK since version 3.7.*
...
typedefvoid(\*PyOS\_sighandler\_t)(int)
:   *Part of the Stable ABI.*

PyOS\_sighandler\_tPyOS\_getsig(inti)
:   *Part of the Stable ABI.*
...
PyOS\_sighandler\_tPyOS\_setsig(inti, PyOS\_sighandler\_th)
:   *Part of the Stable ABI.*
...
intPyOS\_InterruptOccurred(void)
:   *Part of the Stable ABI.*
...
wchar\_t\*Py\_DecodeLocale(constchar\*arg, size\_t\*size)
:   *Part of the Stable ABI since version 3.7.*
    ...
    This function must not be called before Python is preinitialized and so that the LC\_CTYPE locale is properly configured: see
    the `Py_PreInitialize()` function.

    Decode a byte string from the filesystem encoding and error handler.
    If the error handler is surrogateescape error handler, undecodable bytes are decoded as characters in range
...
char\*Py\_EncodeLocale(constwchar\_t\*text, size\_t\*error\_pos)
:   *Part of the Stable ABI since version 3.7.*

    Encode a wide character string to the filesystem encoding and error
    handler. If the error handler is surrogateescape error handler, surrogate characters in the range U+DC80..U+DCFF are
...
FILE\*Py\_fopen(PyObject\*path, constchar\*mode)
...
PyObject\*PySys\_GetObject(constchar\*name)
:   *Return value: Borrowed reference.* *Part of the Stable ABI.*

    Return the object *name* from the `sys` module or `NULL` if it does
    not exist, without setting an exception.

intPySys\_SetObject(constchar\*name, PyObject\*v)
:   *Part of the Stable ABI.*
...
voidPySys\_WriteStdout(constchar\*format, ...)
:   *Part of the Stable ABI.*
...
voidPySys\_WriteStderr(constchar\*format, ...)
:   *Part of the Stable ABI.*

    As `PySys_WriteStdout()`, but write to `sys.stderr` or *stderr*
    instead.

voidPySys\_FormatStdout(constchar\*format, ...)
:   *Part of the Stable ABI.*
...
voidPySys\_FormatStderr(constchar\*format, ...)
:   *Part of the Stable ABI.*
...
intPySys\_Audit(constchar\*event, constchar\*format, ...)
...
intPySys\_AuditTuple(constchar\*event, PyObject\*args)
:   *Part of the Stable ABI since version 3.13.*
...
intPySys\_AddAuditHook(Py\_AuditHookFunctionhook, void\*userData)
:   Append the callable *hook* to the list of active auditing hooks.
    ...
    The hook function is always called with an attached thread state by
    ...
    typedefint(\*Py\_AuditHookFunction)(constchar\*event,PyObject\*args,void\*userData)
...
voidPy\_FatalError(constchar\*message)
:   *Part of the Stable ABI.*
