...
way; see Platform Considerations below).
So, code compiled for Python 3.10.0 will work on 3.10.8 and vice versa,
but will need to be compiled separately for 3.9.x and 3.11.x.

There are two tiers of C API with different stability expectations:

* Unstable API, may change in minor versions without
...
* Limited API, is compatible across several minor releases.
  When `Py_LIMITED_API` is defined, only this subset is exposed
  from `Python.h`.
...
Contents of the Limited API are listed below.
...
code conforms to the Limited API or the Stable ABI. `Py_LIMITED_API` only covers definitions, but an API also
...
the Stable ABI, these details define a “platform”. They
