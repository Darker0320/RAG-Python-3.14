...
`inspect.get_annotations()`. In Python versions 3.10
...
If for some reason `inspect.get_annotations()` isn’t
...
callables created by `functools.partial()`–may
...
newer is to call `getattr()` with three arguments,
...
call `inspect.get_annotations()` to do this work
for you.

If you’re using Python 3.9 or older, or if for some reason
you can’t use `inspect.get_annotations()`, you’ll need
to duplicate its logic. You’re encouraged to examine the
implementation of `inspect.get_annotations()` in the
...
* If `o` is a wrapped callable using `functools.update_wrapper()`,
  `functools.wraps()`, or `functools.partial()`, iteratively
...
* If `o` is a callable (but not a class), use
  `o.__globals__` as the globals when calling
  `eval()`.
...
* Definitions that aren’t needed at runtime, only imported
  when `typing.TYPE_CHECKING` is true.

If `eval()` attempts to evaluate such values, it will
...
* You should avoid accessing `__annotations__` directly on any object.
  Instead, use `annotationlib.get_annotations()` (Python 3.14+)
  or `inspect.get_annotations()` (Python 3.10+).
