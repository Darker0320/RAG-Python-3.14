...
`Py_InitializeFromConfig()` to inform the interpreter about paths to Python run-time
...
which is constructed using the `PyUnicode_DecodeFSDefault()` data
conversion routine.
...
staticPyMethodDefemb_module_methods[]={
{"numargs",emb_numargs,METH_VARARGS,
"Return the number of arguments received by the process."},
{NULL,NULL,0,NULL}
};

staticstructPyModuleDefemb_module={
