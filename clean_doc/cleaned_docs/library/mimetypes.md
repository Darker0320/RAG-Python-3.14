...
mimetypes.guess\_type(*url*, *strict=True*)
Guess the type of a file based on its filename, path or URL, given by *url*.
    ...
    *Content-Transfer-Encoding* header. The mappings are table driven.
    ...
    Soft deprecated since version 3.13: Passing a file path instead of URL is soft deprecated.
    Use `guess_file_type()` for this.

mimetypes.guess\_file\_type(*path*, *\**, *strict=True*)
Guess the type of a file based on its path, given by *path*.
    Similar to the `guess_type()` function, but accepts a path instead of URL.
    Path can be a string, a bytes object or a path-like object.

    Added in version 3.13.

mimetypes.guess\_all\_extensions(*type*, *strict=True*)
...
mimetypes.guess\_extension(*type*, *strict=True*)
...
mimetypes.read\_mime\_types(*filename*)
...
mimetypes.add\_type(*type*, *ext*, *strict=True*)
...
Similar to the `guess_file_type()` function, using the tables stored
...
```
python-mmimetypes[-h][-e][-l]type[type...]
```
...
$ python-mmimetypes--extensionaudio/aacaudio/opusaudio/futureaudio/x-wav
.aac
.opus
