...
Image: Inheritance diagram showing the classes available in pathlib. The most basic class is PurePath, which has three direct subclasses: PurePosixPath, PureWindowsPath, and Path. Further to these four classes, there are two classes that use multiple inheritance: PosixPath subclasses PurePosixPath and Path, and WindowsPath subclasses PureWindowsPath and Path.
...
Create a new directory at this given path. If *mode* is given, it is
    ...
    *mode* into account (mimicking the POSIX `mkdir -p` command).
    ...
    If *exist\_ok* is false (the default), `FileExistsError` is
    raised if the target directory already exists.

    If *exist\_ok* is true, `FileExistsError` will not be raised unless the given
...
Remove this file or symbolic link. If the path points to a directory,
    ...
    If *missing\_ok* is false (the default), `FileNotFoundError` is
    raised if the path does not exist.

    If *missing\_ok* is true, `FileNotFoundError` exceptions will be
...
4. Like all `PurePath` and `Path` objects, the values returned
