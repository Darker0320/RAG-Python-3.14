...
**Source code:** Lib/importlib/resources/\_\_init\_\_.py
...
`importlib.resources` follows the same security model as the built-in
`open()` function. Passing untrusted inputs to the functions
...
migrating from pkg\_resources to importlib.resources.
...
Given a `Traversable` object representing
...
Open the named resource for binary reading.
    ...
    This function returns a `BinaryIO` object,
...
Open the named resource for text reading.
    ...
    *encoding* and *errors* have the same meaning as in built-in `open()`.
    ...
    This function returns a `TextIO` object,
...
Provides the path to the *resource* as an actual file system path. This
    ...
    For example, the `stat()` method requires
