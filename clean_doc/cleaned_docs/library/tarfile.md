...
* reads and writes `gzip`, `bz2`, `compression.zstd`, and
  `lzma` compressed archives if the respective modules are available.

  If any of these optional modules are missing from
...
Changed in version 3.12: Archives are extracted using a filter,
...
Return a `TarFile` object for the pathname *name*. For detailed
    ...
    *mode* has to be a string of the form `'filemode[:compression]'`, it defaults
    ...
    | `'x:zst'` | Create a tarfile with Zstandard compression. Raise a `FileExistsError` exception if it already exists. |
    ...
    *compresslevel* (default `9`) to specify the compression level of the file.
    ...
    `CompressionParameter`. The keyword argument
    *zstd\_dict* can be passed to provide a `ZstdDict`,
...
Base class for members refused by
    filters.
...
tarfile.GNUTYPE\_LONGNAME
A GNU tar longname `type`.

tarfile.GNUTYPE\_LONGLINK
A GNU tar longlink `type`.

tarfile.GNUTYPE\_SPARSE
...
*class*tarfile.TarFile(*name=None*, *mode='r'*, *fileobj=None*, *format=DEFAULT\_FORMAT*, *tarinfo=TarInfo*, *dereference=False*, *ignore\_zeros=False*, *encoding=ENCODING*, *errors='surrogateescape'*, *pax\_headers=None*, *debug=0*, *errorlevel=1*, *stream=False*)
All following arguments are optional and can be accessed as instance attributes
    ...
    *format* controls the archive format for writing. It must be one of the constants
    ...
    If *ignore\_zeros* is `False`, treat an empty block as the end of the archive.
    ...
    *errorlevel* controls how extraction errors are handled,
    ...
    If *stream* is set to `True` then while reading the archive info about files
...
Extract all members from the archive to the current working directory or
    ...
    If *numeric\_owner* is `True`, the uid and gid numbers from the tarfile
    ...
    Read the Extraction filters section for details.
...
Extract a member from the archive as a file object. *member* may be
    ...
    Changed in version 3.13: The returned `io.BufferedReader` object has the `mode`
...
Added in version 3.12.
    ...
    as a default for the *filter* argument of `extract()`
    and `extractall()`.
...
* The `extract()` or `extractall()` methods will
...
Time of last modification in seconds since the epoch,
    ...
    `extractall()`, causing extraction to skip applying this
    attribute.
...
Never extract archives from untrusted sources without prior inspection.
See also Hints for further verification.
...
* the string `'tar'`: Honor most *tar*-specific features (i.e. features of
  ...
  surprising or malicious. See `tar_filter()` for details.
...
* A callable which will be called for each extracted member with a
  ...
  + return a `TarInfo` object which will be used instead of the metadata
    in the archive, or
  ...
  + raise an exception to abort the operation or skip the member,
    ...
    Note that when extraction is aborted, `extractall()` may leave
...
tarfile.fully\_trusted\_filter(*member*, *path*)
Return *member* unchanged.

    This implements the `'fully_trusted'` filter.

tarfile.tar\_filter(*member*, *path*)
...
tarfile.data\_filter(*member*, *path*)
Implements the `'data'` filter.
    ...
    * Refuse to extract links (hard or soft)
      ...
      This raises `AbsoluteLinkError` or
    ...
    * Set user and group info (`uid`, `gid`, `uname`, `gname`)
    ...
    See Hints for further verification for details.
