...
Mock is designed for use with `unittest` and
...
the *new\_callable* argument to `patch()`.
...
Create a new `Mock` object. `Mock` takes several optional arguments
    ...
    * *spec\_set*: A stricter variant of *spec*. If used, attempting to *set*
      ...
      *spec\_set* will raise an `AttributeError`.
...
The reset\_mock method resets all the call attributes on a mock object:
        ...
        *return\_value* parameter when set to `True` resets `return_value`:
        ...
        *side\_effect* parameter when set to `True` resets `side_effect`:
        ...
        Note that `reset_mock()` *doesn’t* clear the
        `return_value`, `side_effect` or any child attributes you have
...
A non-callable version of `Mock`. The constructor parameters have the same
    meaning of `Mock`, with the exception of *return\_value* and *side\_effect*
...
Waits until the mock is called.
        ...
        `AssertionError` if the call is not performed in time.
...
Waits until the mock is called with the specified arguments.
        ...
        the function raises an `AssertionError` if the call is not performed in time.
...
`return_value` from inside `side_effect`, or return `DEFAULT`:
...
`patch()` acts as a function decorator, class decorator or a context
    ...
    *target* is imported and the specified object replaced with the *new*
    ...
    The *spec* and *spec\_set* keyword arguments are passed to the `MagicMock`
    ...
    *new\_callable* allows you to specify a different class, or callable object,
    ...
    `AsyncMock` if the patched object is asynchronous, to
    `MagicMock` otherwise or to *new\_callable* if specified.
...
If you use *spec* or *spec\_set* and `patch()` is replacing a *class*, then the
...
patch the named member (*attribute*) on an object (*target*) with a mock
    ...
    *new\_callable* have the same meaning as for `patch()`. Like `patch()`,
...
more details about how to change the value of see TEST\_PREFIX.
...
magic methods `__getitem__()`, `__setitem__()`,
`__delitem__()` and either `__iter__()` or
`__contains__()`.
...
Perform multiple patches in a single call. It takes the object to be
    ...
    *new\_callable* have the same meaning as for `patch()`. These arguments will
...
do when it imports `SomeClass` from module a. If we use `patch()` to mock out
...
A non-callable version of `MagicMock`.
    ...
    `MagicMock`, with the exception of *return\_value* and
...
Changed in version 3.8: `create_autospec()` now returns an `AsyncMock` if the target is
an async function.
...
`ANY` is not limited to comparisons with call objects and so
...
respond to `dir()`. The default is `True`,
...
unittest.mock.mock\_open(*mock=None*, *read\_data=''*)
A helper function to create a mock to replace the use of `open()`. It works
    ...
    *read\_data* is a string for the `read()`,
    ...
    *read\_data* until it is depleted. The mock of these methods is pretty
    ...
    The mock of `read()` changed to consume *read\_data* rather
    than returning it on each call.

    Changed in version 3.5: *read\_data* is now reset on each call to the *mock*.

    Changed in version 3.8: Added `__iter__()` to implementation so that iteration
...
`Mock` is a very powerful and flexible object, but it suffers from a flaw which
...
ignoring `return_value` and the wrapped object altogether. If any
...
As `None` is the default value of `side_effect`, if you reassign
its value back to `None`, the order of precedence will be checked between
`return_value` and the wrapped object, ignoring
`side_effect`.
...
If the value being returned by `side_effect` is `DEFAULT`,
