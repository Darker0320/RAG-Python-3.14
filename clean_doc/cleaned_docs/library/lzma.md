...
module. Note that `LZMAFile` and `bz2.BZ2File` are *not*
...
`'rb'` for reading and `'wb'` for writing.
    ...
    name
The lzma file name. Equivalent to the `name`
...
### may be buffered internally, for use in later calls to
...
lzma.is\_check\_supported(*check*)
