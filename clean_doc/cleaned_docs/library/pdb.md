...
running without the debugger using the `continue` command.

Changed in version 3.7: The built-in `breakpoint()`, when called with defaults, can be used
...
Changed in version 3.13: The implementation of **PEP 667** means that name assignments made via `pdb`
...
pdb.set\_trace(*\**, *header=None*, *commands=None*)
Enter the debugger at the calling stack frame. This is useful to hard-code
    ...
    *header* is printed to the console just before debugging begins.
    ...
    Changed in version 3.13: `set_trace()` will enter the debugger immediately, rather than
    on the next line of code to be executed.

    Added in version 3.14: The *commands* argument.

*awaitable* pdb.set\_trace\_async(*\**, *header=None*, *commands=None*)
async version of `set_trace()`. This function should be used inside an
...
pdb.set\_default\_backend(*backend*)
...
With a *lineno* argument, set a break at line *lineno* in the current file.
    ...
    yet). The file is searched on `sys.path`. Acceptable forms of *filename*
...
Specify a list of commands for breakpoint number *bpnumber*. The commands
    ...
    the `continue` command, or `step`,
    or any other command that resumes execution.

    Specifying any command resuming execution
    (currently `continue`, `step`, `next`,
    `return`, `until`, `jump`, `quit` and their abbreviations)
...
Start an interactive interpreter (using the `code` module) in a new
    ...
    Changed in version 3.13: `exit()` and `quit()` can be used to exit the `interact`
    command.

    Changed in version 3.13: `interact` directs its output to the debugger’s
...
Restart the debugged Python program. If *args* is supplied, it is split
    ...
    Changed in version 3.14: `run` and `restart` commands are disabled when the
    debugger is invoked in `'inline'` mode.

q(uit)
Quit from the debugger. The program being executed is aborted.
    An end-of-file input is equivalent to `quit`.
    ...
    `sys.exit()` immediately, instead of raising `bdb.BdbQuit`
