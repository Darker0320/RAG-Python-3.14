...
An abstract base class for classes that implement
    `__enter__()` and `__exit__()`. A default
...
An abstract base class for classes that implement
    `__aenter__()` and `__aexit__()`. A default
...
Return a context manager that closes *thing* upon completion of the block. This
    ...
    as `urlopen()` would normally be used in a context manager.
...
Return a context manager that suppresses any of the specified exceptions
    ...
    a new group which is created using the original group’s `derive()`
    method.

    Added in version 3.4.

    Changed in version 3.12: `suppress` now supports suppressing exceptions raised as
    part of a `BaseExceptionGroup`.
...
Enters a new context manager and adds its `__exit__()` method to
        ...
        Changed in version 3.11: Raises `TypeError` instead of `AttributeError` if *cm*
...
Similar to `ExitStack.enter_context()` but expects an asynchronous context
        manager.
...
Similar to `ExitStack.push()` but expects either an asynchronous context manager
...
`suppress()`, `redirect_stdout()`, and `chdir()`. Here’s a very
