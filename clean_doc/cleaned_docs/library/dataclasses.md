...
This function is a decorator that is used to add generated
    ...
    * *eq*: If true (the default), an `__eq__()` method will be
      ...
      Changed in version 3.13: The generated `__eq__` method now compares each field individually
    ...
    * *frozen*: If true (the default is `False`), assigning to fields will
      ...
      If `__setattr__()` or `__delattr__()` is defined in the class
      and *frozen* is true, then `TypeError` is raised.
    * *match\_args*: If true (the default is `True`), the
      `__match_args__` tuple will be created from the list of
      non keyword-only parameters to the generated `__init__()` method (even if
    ...
    * *slots*: If true (the default is `False`), `__slots__` attribute
      ...
      If `__slots__` is already defined in the class, then `TypeError`
      is raised.

    > Warning
    >
    > Passing parameters to a base class `__init_subclass__()`
    > when using `slots=True` will result in a `TypeError`.
...
For common and simple use cases, no other functionality is
    ...
    * *hash*: This can be a bool or `None`. If true, this field is
      included in the generated `__hash__()` method. If false,
...
`InitVar[T]` type annotations describe variables that are init-only. Fields annotated with `InitVar`
    ...
    parameters to `__init__()` and an optional
    `__post_init__()`.
...
Creates a new object of the same type as *obj*, replacing
    ...
    Class, raises `TypeError`. If keys in *changes* are not
...
When defined on the class, it will be called by the generated
    `__init__()`, normally as `self.__post_init__()`.
...
If a `field()` specifies a *default\_factory*, it is called with
...
Changed in version 3.11: Instead of looking for and disallowing objects of type `list`,
...
Fields that are assigned descriptor objects as their
