...
Copy the contents of the file-like object *fsrc* to the file-like object *fdst*.
    ...
    *length* value means to copy the data without looping over the source data in
    ...
    you must ensure that you have called `flush()` or
    `close()` on the file-like object before attempting to read
...
shutil.copy(*src*, *dst*, *\**, *follow\_symlinks=True*)
...
shutil.copytree(*src*, *dst*, *symlinks=False*, *ignore=None*, *copy\_function=copy2*, *ignore\_dangling\_symlinks=False*, *dirs\_exist\_ok=False*)
Recursively copy an entire directory tree rooted at *src* to a directory
    ...
    `FileExistsError` is raised. If *dirs\_exist\_ok* is true, the copying
...
shutil.rmtree(*path*, *ignore\_errors=False*, *onerror=None*, *\**, *onexc=None*, *dir\_fd=None*)
Delete an entire directory tree; *path* must point to a directory (but not a
    ...
    *excinfo*, is the exception that was raised. Exceptions raised by *onexc*
    ...
    rmtree example for an example of handling the removal
    ...
    Changed in version 3.13: `rmtree()` now ignores `FileNotFoundError` exceptions for all
    but the top-level path.
    Exceptions other than `OSError` and subclasses of `OSError`
    are now always propagated to the caller.

    rmtree.avoids\_symlink\_attacks
...
Recursively move a file or directory (*src*) to another location and return
    ...
    directory), this method falls back to using *copy\_function*, in which case
    *src* is copied to the destination using *copy\_function* and then removed.
    ...
    *src* and the destination, and will be used to copy *src* to the destination
    ...
    `copytree()` is called, passing it the *copy\_function*. The
    default *copy\_function* is `copy2()`. Using `copy()` as the
...
shutil.chown(*path*, *user=None*, *group=None*, *\**, *dir\_fd=None*, *follow\_symlinks=True*)
...
`os.copy_file_range()` on supported Linux filesystems.
...
shutil.make\_archive(*base\_name*, *format*[, *root\_dir*[, *base\_dir*[, *verbose*[, *dry\_run*[, *owner*[, *group*[, *logger*]]]]]]])
Create an archive file (such as zip or tar) and return its name.
    ...
    *base\_dir* is the directory where we start archiving from;
    ...
    to *root\_dir*. See Archiving example with base\_dir for how to
    ...
    *logger* must be an object compatible with **PEP 282**, usually an instance of
    ...
    with `register_archive_format()` do not support the *root\_dir*
...
### is the callable that will be used to create archives. The callable
    will receive the *base\_name* of the file to create, followed by the
    *base\_dir* (which defaults to `os.curdir`) to start archiving from.
    Further arguments are passed as keyword arguments: *owner*, *group*,
    *dry\_run* and *logger* (as passed in `make_archive()`).
...
shutil.unpack\_archive(*filename*[, *extract\_dir*[, *format*[, *filter*]]])
Unpack an archive. *filename* is the full path of the archive.
    ...
    *format* is the archive format: one of “zip”, “tar”, “gztar”, “bztar”,
    ...
    the *extract\_dir* argument, for example, members that have absolute filenames
    ...
    Read the Hints for further verification
...
### is the callable that will be used to unpack archives. The
...
shutil.unregister\_unpack\_format(*name*)
Unregister an unpack format. *name* is the name of the format.

shutil.get\_unpack\_formats()
...
we show how to use `make_archive()`, but this time with the usage of
...
Get the size of the terminal window.
    ...
    Changed in version 3.11: The `fallback` values are also used if `os.get_terminal_size()`
    returns zeroes.
