...
Subtypes `Real` and adds `numerator` and
    `denominator` properties. It also provides a default for
    `float()`.

    The `numerator` and `denominator` values
    should be instances of `Integral` and should be in lowest terms with
...
Subtypes `Rational` and adds a conversion to `int`. Provides
    ...
    `denominator`. Adds abstract methods for `pow()` with
