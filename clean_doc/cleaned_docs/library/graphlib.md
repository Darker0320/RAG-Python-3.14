...
Provides functionality to topologically sort a graph of hashable nodes.
    ...
    * While `is_active()` is `True`, iterate over
      the nodes returned by `get_ready()` and
      process them. Call `done()` on each node as it
      finishes processing.
...
Mark the graph as finished and check for cycles in the graph. If any cycle
        is detected, `CycleError` will be raised, but
        `get_ready()` can still be used to obtain as many
        ...
        not started. Previously this raised `ValueError`.
...
Returns `True` if more progress can be made and `False` otherwise.
        ...
        `TopologicalSorter.done()` is less than the number that have been
        returned by `TopologicalSorter.get_ready()`.
...
Marks a set of nodes returned by `TopologicalSorter.get_ready()` as
        processed, unblocking any successor of each node in *nodes* for being
        returned in the future by a call to `TopologicalSorter.get_ready()`.
        ...
        the graph by using `TopologicalSorter.add()`, if called without
        calling `prepare()` or if node has not yet been
...
Returns a `tuple` with all the nodes that are ready. Initially it
        ...
        processed by calling `TopologicalSorter.done()`, further calls will
...
Returns an iterator object which will iterate over nodes in a topological
        order. When using this method, `prepare()` and
        `done()` should not be called. This method is
        equivalent to:
        ...
        `get_ready()`) and the order between them is
