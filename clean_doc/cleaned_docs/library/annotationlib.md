...
* `VALUE` evaluates the annotations and returns their value.
...
* `FORWARDREF` returns `ForwardRef` objects
...
* `STRING` returns the annotations as a string, similar
...
and currently still depends on a future import.
...
An `IntEnum` describing the formats in which annotations
...
Special value used to signal that an annotate function is being
        ...
        the `Format.VALUE` format, or raise `NotImplementedError`
...
A proxy object for forward references in annotations.
    ...
    Instances of this class are returned when the `FORWARDREF`
...
Evaluate the forward reference, returning its value.
        ...
        If the *format* argument is `VALUE` (the default),
        this method may throw an exception, such as `NameError`, if the forward
        ...
        be undefined. If the *format* argument is `FORWARDREF`,
        ...
        is evaluated. *globals* and *locals* are passed to `eval()`, representing
        ...
        It is a tuple of type parameters that are in scope
        ...
        `ForwardRef` instances returned by `get_annotations()`
...
Convert an annotations dict containing runtime values to a
    ...
    annotate functions that support the `STRING` format but
...
Call the annotate function *annotate* with the given *format*,
    a member of the `Format` enum, and return the annotations
    ...
    the `VALUE` format when called directly.
    ...
    *owner* is the object that owns the annotation function, usually
    ...
    `FORWARDREF` format to produce a `ForwardRef`
...
Compute the annotations dict for an object.
    ...
    * FORWARDREF: If `object.__annotations__` exists and can be evaluated successfully,
      ...
      + When calling `object.__annotate__` it is first called with `FORWARDREF`.
        If this is not implemented, it will then check if `VALUE_WITH_FAKE_GLOBALS`
        is supported and use that in the fake globals environment.
        If neither of these formats are supported, it will fall back to using `VALUE`.
    ...
    * STRING: If `object.__annotate__` exists, it is called first;
      ...
      + When calling `object.__annotate__` it is first called with `STRING`.
        If this is not implemented, it will then check if `VALUE_WITH_FAKE_GLOBALS`
        is supported and use that in the fake globals environment.
        If neither of these formats are supported, it will fall back to using `VALUE`
    ...
    * If *eval\_str* is true, values of type `str` will
      ...
      to set *eval\_str* to true with formats other than `Format.VALUE`.
    ...
    * If eval\_str is true, `eval()` is called on values of type
...
Convert an arbitrary Python value to a format suitable for use by the
    `STRING` format. This calls `repr()` for most
...
classClassVarSeparator(type):
   def__new__(mcls, name, bases, ns):
...
classAnnotate:
    ...
    def__call__(self, format=None, /, *, _self=None):
...
```
>>> fromannotationlibimport call_annotate_function, Format
...
```
>>> fromannotationlibimport get_annotations, Format
>>> classC:
...   pass
...
```
>>> deff(x: (1).__class__.__base__.__subclasses__()[-1].__init__.__builtins__["print"]("Hello world")): pass
...
