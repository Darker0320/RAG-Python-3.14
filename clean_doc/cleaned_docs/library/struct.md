...
as Python `bytes` objects. Compact format strings
...
special characters control the byte order, size and alignment.
...
| `b` | signedchar | int | 1 | (2) |
| `B` | unsignedchar | int | 1 | (2) |
| `?` | \_Bool | bool | 1 | (1) |
| `h` | short | int | 2 | (2) |
| `H` | unsignedshort | int | 2 | (2) |
| `i` | int | int | 4 | (2) |
| `I` | unsignedint | int | 4 | (2) |
| `l` | long | int | 4 | (2) |
| `L` | unsignedlong | int | 4 | (2) |
| `q` | longlong | int | 8 | (2) |
| `Q` | unsignedlonglong | int | 8 | (2) |
| `n` | `ssize_t` | int | (2), (3) |
| `N` | `size_t` | int | (2), (3) |
...
6. The IEEE 754 binary16 “half precision” type was introduced in the 2008
   ...
   it’s available as \_Float16 type, if the compiler supports the Annex H
...
8. The `'p'` format character encodes a “Pascal string”, meaning a short
   ...
   `count` bytes, but that the `bytes` object returned can never contain more than 255
   bytes.
   When packing, arguments of types `bytes` and `bytearray`
   are accepted.
9. For the `'s'` format character, the count is interpreted as the length of the
   ...
   to or from ten different Python byte objects. (See Examples
