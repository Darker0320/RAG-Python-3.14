...
module Python
{
    ...
    expr = BoolOp(boolopop, expr* values)
         ...
         | Interpolation(exprvalue, constantstr, intconversion, expr? format_spec)
...
This is the base of all AST node classes. The actual node classes are
    ...
    above. They are defined in the `_ast` C
...
The `_field_types` attribute on each concrete class is a dictionary
        mapping field names (as also listed in `_fields`) to their types.
...
Instances of `ast.expr` and `ast.stmt` subclasses have
        `lineno`, `col_offset`, `end_lineno`, and
        `end_col_offset` attributes. The `lineno` and `end_lineno`
    ...
    * If there are keyword arguments, they will set the attributes of the same
    ...
    `Load()`. If any other field is omitted, a `DeprecationWarning` is raised
...
Changed in version 3.14: The `__repr__()` output of `AST` nodes includes
...
A single interactive input, like in Interactive Mode.
...
A `match` statement. `subject` holds the subject of the match (the object
    ...
    `match_case` nodes with the different cases.
...
Type parameters can exist on classes, functions, and type
aliases.
...
`async for` loops and `async with` context managers. They have the same
    fields as `For` and `With`, respectively. Only valid in the
...
Handles all constant nodes.
    ...
    in Python 3.14+. Add the `visit_Constant()` method instead to handle
    all constant nodes.
