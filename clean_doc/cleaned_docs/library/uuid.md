...
for example, `uuid1()` for UUID version 1, `uuid3()` for UUID version 3, and so on.
...
Generate a pseudo-random UUID according to
    **RFC 9562, §5.8**.
    ...
    secure pseudo-random number generator (CSPRNG). Use `uuid4()` when
...
--uuid <uuid>
Specify the function name to use to generate the uuid. By default `uuid4()`
    is used.
