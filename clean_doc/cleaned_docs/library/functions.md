...
When awaited, return the next item from the given asynchronous
    ...
    This calls the `__anext__()` method of *async\_iterator*,
...
Create a new dictionary. The `dict` object is the dictionary class.
    See also Mapping Types — dict for documentation about this class.
...
Convert a *value* to a “formatted” representation, as controlled by
    *format\_spec*. The interpretation of *format\_spec* will depend on the type
    ...
    `object` and the *format\_spec* is non-empty, or if either the
...
Return a new `frozenset` object, optionally with elements taken from
    *iterable*. `frozenset` is a built-in class. See also
...
Open *file* and return a corresponding file object. If the file
    ...
    * When writing output to the stream, if *newline* is `None`, any `'\n'`
    ...
    The following example uses the dir\_fd parameter of the
...
Return a new `set` object, optionally with elements taken from
    *iterable*. `set` is a built-in class. See also
    Set Types — set, frozenset for documentation about this class.
...
Return a proxy object that delegates method calls to a parent or sibling
    ...
    The *object\_or\_type* determines the method resolution order
    to be searched. The search starts from the class right after the
    *type*.

    For example, if `__mro__` of *object\_or\_type* is
