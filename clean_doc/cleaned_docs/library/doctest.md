...
There is also a command line shortcut for running `testmod()`, see section
Command-line Usage.
...
There is also a command line shortcut for running `testfile()`, see section
Command-line Usage.
...
Detailed report of all examples tried is printed to standard output,
    ...
    that it must be run with `testfile()` instead:
...
All arguments are optional, and all except for *m* should be specified in
    ...
    tests. The *exclude\_empty* argument to the newer `DocTestFinder`
    constructor defaults to true.

    Optional arguments *extraglobs*, *verbose*, *report*, *optionflags*,
    *raise\_on\_error*, and *globs* are the same as for function `testfile()`
...
a load\_tests function in your test module:
...
Convert doctest tests from one or more text files to a
    ...
    * If *module\_relative* is `False`, then each filename in *paths* specifies
    ...
    access the test globals as the `globs` attribute of the test
    passed.
...
Convert doctest tests for a module to a `unittest.TestSuite`.
    ...
    and a `unittest.TestCase.failureException` exception is raised
    ...
    Optional argument *globs* is a dictionary containing the initial global
    variables for the tests. A new copy of this dictionary is created for each
    test. By default, *globs* is the module’s `__dict__`.
    ...
    Optional argument *test\_finder* is the `DocTestFinder` object (or a
...
Return a list of the `DocTest`s that are defined by *obj*’s
        ...
        *extraglobs* (bindings in *extraglobs* override bindings in *globs*). A new
        ...
        `__dict__`, if specified, or `{}` otherwise.
...
Print a summary of all the test cases that have been run by this DocTestRunner,
        and return a `TestResults` instance.
