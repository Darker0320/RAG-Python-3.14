...
For instance `write()` returns the number of bytes written
which may be less than the number of bytes provided (a partial write).
High-level I/O objects like Binary I/O and Text I/O implement
retry behavior.
...
Read and return all the bytes from the stream until EOF, using multiple
        ...
        non-blocking mode and the underlying `read()` returns `None`
...
Write the given bytes-like object, *b*, to the
        ...
        objects like Binary I/O and Text I/O implement retry behavior.
...
A raw binary stream representing an OS-level file containing bytes data. It
    ...
    This means `write()` does not guarantee all bytes are written
    and `read()` may read less bytes than requested even when more
    bytes may be present in the underlying file. To get “write all” and
    “read at least” behavior, use Binary I/O.
    ...
    * an integer representing the number of an existing OS-level file descriptor
    ...
    `read()` and `write()`, need to have their
    return values checked explicitly in a retry loop to implement “write all”
    and “read at least” behavior. High-level I/O objects Binary I/O and
    Text I/O implement retry behavior.
...
A binary stream using an in-memory bytes buffer. It inherits from
    ...
    The optional argument *initial\_bytes* is a bytes-like object that
...
A buffered text stream providing higher-level access to a
    ...
    * When writing output to the stream, if *newline* is `None`, any `'\n'`
    ...
    If *line\_buffering* is `True`, `flush()` is implied when a call to
    write contains a newline character or a carriage return.

    If *write\_through* is `True`, calls to `write()` are guaranteed
...
Write *data* to the output stream and return the number of items
    ...
    ```
    defwrite_binary(writer: Writer[bytes]):
