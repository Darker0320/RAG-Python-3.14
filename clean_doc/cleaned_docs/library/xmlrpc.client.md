...
*class*xmlrpc.client.ServerProxy(*uri*, *transport=None*, *encoding=None*, *verbose=False*, *allow\_none=False*, *use\_datetime=False*, *use\_builtin\_types=False*, *\**, *headers=()*, *context=None*)
A `ServerProxy` instance is an object that manages communication with a
    ...
    If an HTTPS URL is provided, *context* may be `ssl.SSLContext`
    ...
    | `dateTime.iso8601` | `DateTime` or `datetime.datetime`. Returned type depends on values of *use\_builtin\_types* and *use\_datetime* flags. |
    | `base64` | `Binary`, `bytes` or `bytearray`. Returned type depends on the value of the *use\_builtin\_types* flag. |
