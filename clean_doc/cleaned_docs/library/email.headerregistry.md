...
This is the factory used by `EmailPolicy` by default.
    ...
    dynamically, using *base\_class* and a specialized class retrieved from a
    registry that it holds. When a given header name does not appear in the
    registry, the class specified by *default\_class* is used as the specialized
    class. When *use\_default\_map* is `True` (the default), the standard
    mapping of header names to classes is copied in to the registry during
    initialization. *base\_class* is always the last class in the generated
...
*name* is the name of the header to be mapped. It will be converted to
        ...
        along with *base\_class*, to create the class used to instantiate headers
        that match *name*.
...
Retrieves the specialized header associated with *name* from the
        registry (using *default\_class* if *name* does not appear in the
        registry) and composes it with *base\_class* to produce a class,
...
The class used to represent an email address. The general form of an
    ...
    **RFC 5322**.
    ...
    As a convenience *addr\_spec* can be specified instead of *username* and
    *domain*, in which case *username* and *domain* will be parsed from the
    *addr\_spec*. An *addr\_spec* must be a properly RFC quoted string; if it is
...
The class used to represent an address group. The general form of an
    ...
    single addresses that are not part of a group by setting *display\_name* to
