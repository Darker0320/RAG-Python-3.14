...
Get a regular expression that can be used with the `regex(3)` function to
        ...
        The regular expressions for `YESEXPR` and
        `NOEXPR` use syntax suitable for the
...
Get a string which describes how years are counted and displayed for
        ...
        Specifications Issue 8*, paragraph 7.3.5.2 LC\_TIME C-Language Access.
...
Return the locale encoding used for text data, according to user
    ...
    return `'utf-8'`, the locale encoding and the *do\_setlocale*
    argument are ignored.

    The Python preinitialization configures the LC\_CTYPE
...
Converts a string to a number, following the `LC_NUMERIC` settings,
    by calling *func* on the result of calling `delocalize()` on *string*.
...
locale.LC\_NUMERIC
Locale category for formatting numbers. The functions `format_string()`,
...
On Windows, several formats are supported. [2] [3]
A subset of IETF BCP 47 tags:
