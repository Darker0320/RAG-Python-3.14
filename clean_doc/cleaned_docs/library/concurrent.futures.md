...
and Lib/concurrent/futures/interpreter.py
...
Signal the executor that it should free any resources that it is using
        ...
        as a context manager via the `with` statement, which
...
An `Executor` subclass that uses a pool of at most *max\_workers*
    threads to execute calls asynchronously.
...
The `InterpreterPoolExecutor` class uses a pool of interpreters
...
Global Interpreter Lock, so code
...
interpreter, including `sys`, `builtins`,
and even `__main__`.
...
efficient alternative is to serialize with `pickle` and then send
the bytes over a shared `socket` or
`pipe`.
...
`submit()` and `map()` work like normal,
except the worker serializes the callable and arguments using
`pickle` when sending them to its interpreter. The worker
...
An `Executor` subclass that executes calls asynchronously using a pool
    ...
    If *max\_workers* is less than or equal to `0`, then a `ValueError`
    will be raised.
    ...
    available.
    *mp\_context* can be a `multiprocessing` context or `None`. It will be
    ...
    *max\_tasks\_per\_child* is an optional argument that specifies the maximum
    ...
    `DeprecationWarning`. Pass a *mp\_context* configured to use a
    different start method. See the `os.fork()` documentation for
    further explanation.

    Changed in version 3.13: *max\_workers* uses `os.process_cpu_count()` by default, instead of
    `os.cpu_count()`.
    ...
    Changed in version 3.14.6 (unreleased): Fixed a deadlock (gh-115634) where the executor could hang after
    a worker process exited upon reaching its *max\_tasks\_per\_child*
    limit while tasks remained queued.

    terminate\_workers()
Attempt to terminate all living worker processes immediately by calling
        `Process.terminate` on each of them.
        Internally, it will also call `Executor.shutdown()` to ensure that all
...
Wait for the `Future` instances (possibly created by different
    ...
    *return\_when* indicates when this function should return. It must be one of
    ...
    | concurrent.futures.FIRST\_EXCEPTION | The function will return when any future finishes by raising an exception. If no future raises an exception then it is equivalent to `ALL_COMPLETED`. |
...
Derived from `BrokenThreadPool`,
    this exception class is raised when one of the workers
    of a `InterpreterPoolExecutor`
    has failed initializing.
