...
This is a subclass derived from `IMAP4` that connects over an SSL
    ...
    *ssl\_context* is a `ssl.SSLContext` object which allows bundling
    ...
    With the default *ssl\_context*, the connection is encrypted but the
...
Force use of `CRAM-MD5` authentication when identifying the client to protect
    ...
    Changed in version 3.14: An `IMAP4.error` is raised if MD5 support is not available.
