...
shlex.join(*split\_command*)
Concatenate the tokens of the list *split\_command* and return a string.
    This function is the inverse of `split()`.

    ```
    >>> fromshleximport join
...
A `shlex` instance or subclass instance is a lexical analyzer
    ...
    parsing rules. The *punctuation\_chars* argument provides a way to make the
    ...
    *punctuation\_chars* will be removed from `wordchars`. See
    Improved Compatibility with Shells for more information. *punctuation\_chars*
    can be set only upon `shlex` instance creation and can’t be
    modified later.

    Changed in version 3.6: The *punctuation\_chars* parameter was added.
...
The string of characters that will accumulate into multi-character tokens. By
    ...
    there. If `whitespace_split` is set to `True`, this will have no
    effect.
...
If `True`, tokens will only be split in whitespaces. This is useful, for
    ...
    Changed in version 3.8: The `punctuation_chars` attribute was made compatible with the
...
When operating in POSIX mode, `shlex` will try to obey the
...
* Enclosing characters in quotes which are part of
  ...
  mentioned in `escape`. The escape characters retain their
...
Instead of passing `True` as the value for the punctuation\_chars parameter,
