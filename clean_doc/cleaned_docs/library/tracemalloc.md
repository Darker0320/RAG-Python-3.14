...
tracemalloc.clear\_traces()
Clear traces of memory blocks allocated by Python.

    See also `stop()`.

tracemalloc.get\_object\_traceback(*obj*)
...
tracemalloc.get\_traceback\_limit()
...
tracemalloc.get\_traced\_memory()
...
Filename pattern of the filter (`str`). Read-only property.
    ...
    all\_frames
If *all\_frames* is `True`, all frames of the traceback are checked. If
        *all\_frames* is `False`, only the most recent frame is checked.
...
Compute the differences with an old snapshot. Get statistics as a sorted
        list of `StatisticDiff` instances grouped by *key\_type*.

        See the `Snapshot.statistics()` method for *key\_type* and *cumulative*
        parameters.
...
Get statistics as a sorted list of `Statistic` instances grouped
        ...
        The cumulative mode can only be used with *key\_type* equal to
        `'filename'` and `'lineno'`.
...
Format the traceback as a list of lines. Use the `linecache` module to
        ...
        `abs(limit)` oldest frames. If *most\_recent\_first* is `True`, the order
