...
**Source code:** Lib/multiprocessing/shared\_memory.py
...
processes, a `BaseManager` subclass,
...
Create an instance of the `SharedMemory` class for either
    ...
    Parameters:
* **name** (*str* *|* *None*) – The unique name for the requested shared memory, specified as a string.
          When creating a new shared memory block, if `None` (the default)
          is supplied for the name, a novel name will be generated.
        * **create** (*bool*) – Control whether a new shared memory block is created (`True`)
          or an existing shared memory block is attached (`False`).
        * **size** (*int*) – The requested number of bytes when creating a new shared memory block.
        ...
        * **track** (*bool*) – When `True`, register the shared memory block with a resource
...
Delete the underlying shared memory block. This should be called only
        ...
        `unlink()` and `close()` can be called in any order, but
        trying to access data inside a shared memory block after `unlink()`
...
A subclass of `multiprocessing.managers.BaseManager` which can be
    ...
    Refer to `BaseManager` for a description
...
`SharedMemoryManager` objects via the
`with` statement to ensure that all shared memory blocks are released
...
in a `with` statement, the shared memory blocks created using that
