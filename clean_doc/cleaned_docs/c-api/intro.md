...
Py\_GETENV(s)
Like `getenv(s)`, but returns `NULL` if `-E` was passed
    on the command line (see `PyConfig.use_environment`).
...
Creates a variable with name *name* that can be used in docstrings.
    ...
    staticPyMethodDefdeque_methods[]={
    // ...
...
Expands to the given input string, or an empty string
    if docstrings are disabled (`--without-doc-strings`).

    Example:

    ```
    staticPyMethodDefpysqlite_row_methods[]={
...
Return the absolute value of `x`.
    ...
    `INT_MIN` value for int type), the behavior is
    undefined.
...
Py\_ARITHMETIC\_RIGHT\_SHIFT(type, integer, positions)
...
Py\_SAFE\_DOWNCAST(value, larger, smaller)
...
Ask the compiler to always inline a static inline function. The compiler can
    ...
    If Python is built in debug mode (if the `Py_DEBUG`
    macro is defined), the `Py_ALWAYS_INLINE` macro does nothing.
...
Py\_LOCAL\_INLINE(type)
Equivalent to `Py_LOCAL` but additionally requests the function
    be inlined.
...
Py\_MEMCPY(dest, src, n)
This is a soft deprecated alias to `memcpy()`.
    Use `memcpy()` directly instead.
...
places there are that have a strong reference to an object.
Such a place could be another object, or a global (or static) C variable,
or a local variable in some C function.
When the last strong reference to an object is released
...
It is not necessary to hold a strong reference (i.e. increment
...
our variable, there is no need to take a new strong reference
...
intrv=-1;/* Return value initialized to -1 (failure) */
...
returnrv;/* -1 for error, 0 for success */
}
```
