...
PyObject\*PyBytes\_FromStringAndSize(constchar\*v, Py\_ssize\_tlen)
*Return value: New reference.* *Part of the Stable ABI.* *Thread safety: Atomic.*
...
PyObject\*PyBytes\_FromFormat(constchar\*format, ...)
*Return value: New reference.* *Part of the Stable ABI.* *Thread safety: Atomic.*
    ...
    *format* string. The following format characters are allowed:
    ...
    | `%zu` | size\_t | Equivalent to `printf("%zu")`. [1] |
...
PyObject\*PyBytes\_FromFormatV(constchar\*format, va\_listvargs)
*Return value: New reference.* *Part of the Stable ABI.* *Thread safety: Atomic.*
...
Py\_ssize\_tPyBytes\_Size(PyObject\*o)
*Part of the Stable ABI.* *Thread safety: Atomic.*

    Return the length of the bytes in bytes object *o*.

Py\_ssize\_tPyBytes\_GET\_SIZE(PyObject\*o)
...
intPyBytes\_AsStringAndSize(PyObject\*obj, char\*\*buffer, Py\_ssize\_t\*length)
*Part of the Stable ABI.* *Thread safety: Safe to call from multiple threads with external synchronization only.*
...
voidPyBytes\_Concat(PyObject\*\*bytes, PyObject\*newpart)
*Part of the Stable ABI.* *Thread safety: Safe for concurrent use on the same object.*
    ...
    The reference to the old value of *bytes* will be “stolen”.
...
voidPyBytes\_ConcatAndDel(PyObject\*\*bytes, PyObject\*newpart)
*Part of the Stable ABI.* *Thread safety: Safe for concurrent use on the same object.*

    Create a new bytes object in *\*bytes* containing the contents of *newpart*
    appended to *bytes*. This version releases the strong reference
...
PyObject\*PyBytes\_Join(PyObject\*sep, PyObject\*iterable)
...
int\_PyBytes\_Resize(PyObject\*\*bytes, Py\_ssize\_tnewsize)
...
PyObject\*PyBytes\_Repr(PyObject\*bytes, intsmartquotes)
*Part of the Stable ABI.* *Thread safety: Atomic.*
...
PyObject\*PyBytes\_DecodeEscape(constchar\*s, Py\_ssize\_tlen, constchar\*errors, Py\_ssize\_tunicode, constchar\*recode\_encoding)
*Part of the Stable ABI.* *Thread safety: Atomic.*
    ...
    *errors* is `NULL`, then `"strict"` is used by default.
    ...
    Changed in version 3.9: *unicode* and *recode\_encoding* are now unused.
