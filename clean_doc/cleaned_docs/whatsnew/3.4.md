...
versions of those guides remaining available as Building C and C++ Extensions with setuptools
and Building C and C++ Extensions with setuptools.
...
```
>>> fromcodecsimport encode, decode
>>> encode(b"hello", "hex")
b'68656c6c6f'
...
mmap objects are now weakly referenceable.
(Contributed by Valerie Lambert in bpo-4885.)
...
so you can use it to create `Pool`s and other
...
New `fullmatch()` function and `Pattern.fullmatch()` method anchor
...
(`crl`s), as well as a `get_ca_certs()` method that
...
`getoutput()` and `getstatusoutput()` now
...
context manager can be queried for the `LogRecord`s and/or
...
The `getparams()` method now returns a namedtuple rather
...
*short\_empty\_elements* keyword-only parameter
...
* A new opcode, `LOAD_CLASSDEREF`, has been added to fix a bug in the
  ...
  uses of \_\_prepare\_\_. (Contributed by Benjamin Peterson in
  bpo-17853.)
...
* Running IDLE — Python editor and shell with the `-n` flag (no subprocess) is deprecated.
...
* `object.__format__()` no longer accepts non-empty format strings, it now
  ...
  that do not have a \_\_format\_\_ method that handles it. See bpo-7994 for
  background.
...
* The [X refs, Y blocks] output of a debug (`--with-pydebug`) build of the
  ...
  `-X showrefcount` option. (Contributed by Ezio Melotti in bpo-17323.)
...
* The module type now initializes the `__package__` and
  `__loader__`
...
* Because `unittest.TestSuite` now drops references to tests after they
  are run, test harnesses that reuse a `TestSuite` to re-run
...
* The new *convert\_charrefs* argument to `HTMLParser`
