...
`__rmatmul__()`, and `__imatmul__()` for regular,
...
a \_\_future\_\_ import is necessary:
...
allow\_abbrev to `False`. (Contributed by Jonathan Paugh,
...
and `OrderedDict.values()` views now support `reversed()` iteration.
...
The `signature()` function now accepts a *follow\_wrapped*
...
*exist\_ok* argument to match `mkdir -p` and `os.makedirs()`
...
The `move()` function now accepts a *copy\_function* argument,
...
(**RFC 6152**) if *decode\_data* has been set `True`. If the client
...
The `MagicMock` class now supports
`__truediv__()`, `__divmod__()`
and `__matmul__()` operators.
...
* The deprecated “strict” mode and argument of `HTMLParser`,
  ...
  The *convert\_charrefs* argument of `HTMLParser` is
...
* Because the lack of the `__module__` attribute breaks pickling and
  ...
  the `__module__` attribute. This will be an `AttributeError` in
  the future.
