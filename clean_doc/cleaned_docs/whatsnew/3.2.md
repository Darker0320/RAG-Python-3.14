...
* Previously it was illegal to delete a name from the local namespace if it
  ...
  (See bpo-4617.)
* Struct sequence types are now subclasses of tuple.
...
* `range` objects now support *index* and *count* methods. This is part
  ...
  (Contributed by Daniel Stutzbach in bpo-9213, by Alexander Belopolsky
  in bpo-2690, and by Nick Coghlan in bpo-10889.)
...
* The `functools.wraps()` decorator now adds a `__wrapped__` attribute
  ...
  be introspected. It also copies `__annotations__` if
...
* To help write classes with rich comparison methods, a new decorator
  ...
  `total_ordering()` to fill-in *\_\_le\_\_*, *\_\_gt\_\_* and *\_\_ge\_\_*:
  ...
  With the *total\_ordering* decorator, the remaining comparison methods
...
function. See https://en.wikipedia.org/wiki/Gamma\_function for details. Because
...
* When linked against recent versions of OpenSSL, the `ssl` module now
  ...
  the *server\_hostname* argument to `ssl.SSLContext.wrap_socket()`.
...
`HTTPSConnection` classes now have a *source\_address*
...
* Experimentation at the interactive prompt is now easier because the
  `unittest.TestCase` class can now be instantiated without
  arguments:
...
* Serializing and unserializing data using the `pickle` module is now
  ...
  and the Unladen Swallow team in bpo-9410 and bpo-3873.)
...
* JSON decoding performance is improved and memory consumption is reduced
  ...
  (Contributed by Antoine Pitrou in bpo-7451 and by Raymond Hettinger and
  Antoine Pitrou in bpo-10314.)
...
* The `array.tostring()` and `array.fromstring()` have been renamed to
  `array.tobytes()` and
  `array.frombytes()` for clarity. The old names
...
* In `subprocess.Popen`, the default value for *close\_fds* is now
