...
```
defgreet_all(names: list[str]) -> None:
    for name in names:
        print("Hello", name)
```
...
`asyncio` now raises `TypeError` when calling incompatible
...
`http.HTTPStatus`. (Contributed by Donghee Na in bpo-39509 and Ross Rhodes in bpo-39507.)
...
* A number of Python builtins (`range`, `tuple`, `set`,
  ...
  **PEP 590** vectorcall protocol.
  (Contributed by Donghee Na, Mark Shannon, Jeroen Demeyer and Petr Viktorin in bpo-37207.)
...
* floor division of float operation now has a better performance. Also
  ...
  (Contributed by Donghee Na in bpo-39434.)
...
* `nntplib.NNTP`: `xpath()` and `xgtitle()` methods have been removed.
  ...
  (Contributed by Donghee Na in bpo-39366.)
...
* The old `plistlib` API has been removed, it was deprecated since Python
  ...
  `dumps()` functions. Additionally, the *use\_builtin\_types* parameter was
...
* Instances of heap-allocated types (such as those created with
  ...
  > ```
  > int
  > foo_traverse(PyObject*self,visitprocvisit,void*arg)
  > {
  ...
  Note that only heap type are expected to visit the type
  in `tp_traverse`.
...
* The `PyModule_AddType()` function is added to help adding a type
  to a module.
  (Contributed by Donghee Na in bpo-40024.)
