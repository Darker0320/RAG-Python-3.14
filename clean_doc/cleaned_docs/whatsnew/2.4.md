...
single class called `subprocess.Popen` whose constructor supports a number of
...
* *close\_fds* requests that all file descriptors be closed before running the
  subprocess.
...
* *universal\_newlines* opens the child’s input and output using Python’s
...
* Some other new encodings were added: HP Roman8, ISO\_8859-11, ISO\_8859-16,
...
* A new function, `PyDict_Contains(d, k)`, implements fast dictionary
  ...
  (Contributed by Raymond Hettinger.)
* The Py\_IS\_NAN(X) macro returns 1 if its float or double argument
...
* A new method flag, `METH_COEXIST`, allows a function defined in slots
