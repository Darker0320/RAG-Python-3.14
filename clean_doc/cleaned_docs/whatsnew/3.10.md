...
* **PEP 624**, Remove Py\_UNICODE encoder APIs
...
with additions by Yurii Karabas and Serhiy Storchaka in bpo-44490.)
...
* Builtin and extension functions that take integer arguments no longer accept
  `Decimal`s, `Fraction`s and other
...
* A `SyntaxError` (instead of a `NameError`) will be raised when deleting
  the `__debug__` constant. (Contributed by Donghee Na in bpo-45000.)
...
(Contributed by Raymond Hettinger in bpo-9694.)
...
Add the *root\_dir* and *dir\_fd* parameters in `glob()` and
...
Added importlib.metadata.packages\_distributions()
...
objects in the tree returned by `pyclbr.readmodule()` and
