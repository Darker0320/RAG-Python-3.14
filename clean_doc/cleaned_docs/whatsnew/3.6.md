...
* `python.exe` and `pythonw.exe` have been marked as long-path aware,
  ...
  See removing the MAX\_PATH limitation for details.
...
type.\_\_prepare\_\_()) is now an insertion-order-preserving
mapping.
...
without the need to recompile specific debug builds or
...
`Flag` and `IntFlag`. Both are used to define
...
`IntFlag` class for their constants.
...
* The format of the `co_lnotab` attribute of code objects
  changed to support
...
* The following modules have had missing APIs added to their
  `__all__` attributes to match the documented APIs:
  `calendar`, `cgi`, `csv`,
  `ElementTree`, `enum`,
...
* As part of **PEP 487**, the handling of keyword arguments passed to
  ...
  `type.__new__` and `type.__init__` both now accept arbitrary keyword arguments,
  but `object.__init_subclass__()` (which is called from
  `type.__new__`) will reject them by default.
  Custom metaclasses accepting additional keyword arguments will need to adjust
  their calls to `type.__new__`
  (whether direct or via `super`) accordingly.
...
* The `mmap.mmap.write()` function now returns the number
