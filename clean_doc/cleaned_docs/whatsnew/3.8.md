...
The ABI of Python debug builds is now compatible with
...
for various classes.
Any static type implementing a callable can use this
protocol.
...
```
fromfunctoolsimport singledispatchmethod
...
In all cases, `os.stat_result.st_mode` will only have `S_IFLNK` set for
symbolic links and not other kinds of reparse points. To identify other kinds
of reparse point, check the new `os.stat_result.st_reparse_tag` attribute.
...
The `pprint` module added a *sort\_dicts* parameter to several functions.
...
like `pprint.pprint()` but with *sort\_dicts* defaulting to `False`:

```
>>> frompprintimport pprint, pp
...
`canonicalize()` that implements C14N 2.0.
...
* Many builtin and extension functions that take integer arguments will
  now emit a deprecation warning for `Decimal`s,
  `Fraction`s and any other objects that can be converted
...
* `subprocess.Popen` can now use `os.posix_spawn()` in some cases
  ...
  Emulation, the `Popen` constructor using
  `os.posix_spawn()` no longer raises an exception on errors like
  “missing program”. Instead the child process fails with a
  non-zero `returncode`.
  (Contributed by Joannah Nanjekye and Victor Stinner in bpo-35537.)
* The *preexec\_fn* argument of \* `subprocess.Popen` is no longer
  ...
  in bpo-37951.)
* The `imaplib.IMAP4.logout()` method no longer silently ignores arbitrary
  exceptions.
...
* The `writexml()`, `toxml()`
  and `toprettyxml()` methods of
  `xml.dom.minidom` and the `write()`
  method of `xml.etree.ElementTree` now preserve the attribute order
...
* The `cProfile.Profile` class can now be used as
  a context manager. (Contributed by Scott Sanderson in bpo-29235.)
* `shutil.copyfile()`, `shutil.copy()`, `shutil.copy2()`,
  `shutil.copytree()` and `shutil.move()` use platform-specific
...
* The `PyCompilerFlags` structure got a new *cf\_feature\_version*
