...
bpo-2377 - Replace \_\_import\_\_ w/ importlib.\_\_import\_\_
bpo-13959 - Re-implement parts of `imp` in pure Python
bpo-14605 - Make import machinery explicit
bpo-14646 - Require loaders set \_\_loader\_\_ and \_\_package\_\_
...
`SIGBUS`, and `SIGILL` signals.
You can also enable them at startup by setting the `PYTHONFAULTHANDLER`
environment variable or by using `-X` `faulthandler` command line option.
...
* To avoid race conditions like symlink attacks and issues with temporary
  ...
  + The following functions get new optional *dir\_fd* (paths relative to
    directory descriptors) and/or *follow\_symlinks* (not
    following symlinks):
...
* The `socket` class now exposes additional methods to process
  ...
  Heiko Wundram)
* The `socket` class now supports the PF\_CAN protocol family
...
* The `socket` class now supports the PF\_RDS protocol family
  (https://en.wikipedia.org/wiki/Reliable\_Datagram\_Sockets and
...
`tempfile.SpooledTemporaryFile`'s `truncate()` method now accepts
...
* `platform.popen()`: use the `subprocess` module. Check especially
  the Replacing Older Functions with the subprocess Module section (bpo-11377).
...
* `PyUnicode_EncodeMBCS()`: use `PyUnicode_AsMBCSString()` or
  `PyUnicode_EncodeCodePage()` (with `CP_ACP` code\_page)
