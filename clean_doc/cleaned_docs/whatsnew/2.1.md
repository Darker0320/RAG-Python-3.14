...
instead of `repr()`. For example, you can set it to a special
...
For example, the Distutils **sdist** and **bdist\_\*** commands
...
* Modules can now control which names are imported when `from module import *`
  ...
  modules such as `sys` or `string`, `from module import *` will add
