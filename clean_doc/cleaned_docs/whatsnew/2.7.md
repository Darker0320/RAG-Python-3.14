...
The recommended starting point is the How to port Python 2 Code to Python 3 HOWTO guide.
...
* The `json` module’s `JSONDecoder` class
  constructor was extended with an *object\_pairs\_hook* parameter to
...
* Constructors for the parsing classes in the `ConfigParser` module now
  take an *allow\_no\_value* parameter, defaulting to false; if true,
...
* The default `HTTPResponse` class used by the `httplib` module now
  ...
  now support a *source\_address* parameter, a `(host, port)` 2-tuple
...
* Updated module: The `json` module was upgraded to version 2.0.9 of the
  ...
  now has an optional *object\_pairs\_hook* parameter that will be called
...
* The `create_connection()` function
  gained a *source\_address* parameter, a `(host, port)` 2-tuple
  giving the source address that will be used for the connection.
  (Contributed by Eldon Ziegler; bpo-3972.)
...
* The `zipfile` module’s `ZipFile` now supports the context
  ...
  The `writestr()` method now has an optional *compress\_type* parameter
...
* ElementTree’s code for converting trees to a string has been
  ...
  you can use the true/false *xml\_declaration* parameter to suppress the
  XML declaration.
* New `Element` method:
...
* Because of an optimization for the `with` statement, the special
  ...
  types. (bpo-6101.)
* Due to a bug in Python 2.6, the *exc\_value* parameter to
  `__exit__()` methods was often the string representation of the
  exception, not an instance. This was fixed in 2.7, so *exc\_value*
