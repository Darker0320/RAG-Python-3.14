...
clause splits (see `split()`) it into the subgroups
...
type of the target `e` is consistently `BaseExceptionGroup`:
...
or `continue` statement, the saved exception is discarded. For example,
...
Changed in version 3.8: Prior to Python 3.8, a `continue` statement was illegal in the
`finally` clause due to a problem with the implementation.

Changed in version 3.14: The compiler emits a `SyntaxWarning` when a `return`,
`break` or `continue` appears in a `finally`
block (see **PEP 765**).
...
7. The context manager’s `__exit__()` method is invoked. If an exception
...
the singletons `None`, `True` and `False`, the `is` operator is used.
...
there’s an applicable `global` or `nonlocal` statement.
...
`_` is a soft keyword within any pattern,
...
3. The subsequent steps depend on whether the sequence pattern is fixed or
   ...
   `len()` (i.e. via the `__len__()` protocol).
...
raise a `SyntaxError`. Two keys that otherwise have the same value will
...
3. If duplicate keys are detected in the mapping pattern, the pattern is
   considered invalid. A `SyntaxError` is raised for duplicate literal
   values; or a `ValueError` for named keys of the same value.
...
2. If the subject value is not an instance of `name_or_attr` (tested via
   `isinstance()`), the class pattern fails.
3. If no pattern arguments are present, the pattern succeeds. Otherwise,
   ...
   1. The equivalent of `getattr(cls, "__match_args__", ())` is called.
      ...
      * Otherwise, positional pattern `i` is converted to a keyword pattern
        ...
        be a string; if not `TypeError` is raised.
...
`assignment_expression`. Previously, the grammar was
...
namespace is saved. [5] A class object is then created using the inheritance
...
(see coroutine). `await` expressions, `async for` and
`async with` can only be used in the body of a coroutine function.
...
An asynchronous iterable provides an `__aiter__` method that directly
...
Functions (including coroutines),
classes and type aliases may
...
Thus, after a declaration `def funcT: pass`, the name `T` is not available in
...
* `typing.TypeVar`, introduced by a plain name (e.g., `T`). Semantically, this
...
`typing.TypeVar` declarations can define *bounds* and *constraints* with
...
`typing.TypeVarTuple`s and `typing.ParamSpec`s cannot have bounds
or constraints.
...
Here `annotation-def` indicates an annotation scope,
...
attribute access on the `typing` module, but creates an instance of
`typing.TypeVar` directly.)
...
`TypeVar` bound, this is equivalent to:
...
Except for the lazy evaluation of the value,
this is equivalent to:
...
(such as `dataclasses` or `functools.singledispatch()`).
...
if requested. The `annotationlib` module provides tools for evaluating annotations.

If the future statement `from __future__ import annotations` is present,
