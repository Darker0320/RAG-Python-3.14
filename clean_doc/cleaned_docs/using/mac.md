...
the python.org website. See
Alternative Distributions for some other options.
...
Image: ../_images/mac_installer_01_introduction.png
...
Image: ../_images/mac_installer_02_readme.png
...
Image: ../_images/mac_installer_03_license.png
...
Image: ../_images/mac_installer_04_installation_type.png
...
see Installing Free-threaded Binaries.

Image: ../_images/mac_installer_05_custom_install.png
...
Image: ../_images/mac_installer_08_install_certificates.png
...
* PySide: Official Python bindings to the
  Qt GUI toolkit.
* PyQt: Alternative
  Python bindings to Qt.
* Kivy: A cross-platform GUI toolkit that supports
  desktop and mobile platforms.
* Toga: Part of the BeeWare Project; supports desktop, mobile, web and console apps.
* wxPython: A cross-platform toolkit that
...
with an extension module, may not be ready for use in a
free-threaded build, and will re-enable the GIL.
...
Image: ../_images/mac_installer_09_custom_install_free_threaded.png
...
* The **UNIX command-line tools** package, which is selected by default,
  will install links in `/usr/local/bin` for `python3.14t`,
  the free-threaded interpreter, and `python3.14t-config`,
...
* For this release, the **Shell profile updater** package and the
  `Update Shell Profile.command` in `/Applications/Python 3.14/`
...
* When working with multiple Python environments, it is usually safest and easiest
  to create and use virtual environments.
...
* If you cannot depend on the link in `/usr/local/bin` pointing to the
  ...
  like `python3.14` in both; which one is actually used depends on the order
...
using the `3.14.0b2` release and selecting the free-threaded interpreter
option:
...
* py2app: Supports creating macOS `.app`
...
* Briefcase: Part of the BeeWare Project; a cross-platform packaging tool that supports
  ...
  notarization.
* PyInstaller: A cross-platform packaging tool that creates
...
The python.org Help page has links to many useful resources.
The Pythonmac-SIG mailing list
