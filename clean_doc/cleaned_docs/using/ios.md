...
Python, projects such as BeeWare and Kivy will provide a much more approachable user experience.
...
`platform.ios_ver()`. `platform.system()` will report `iOS` or
...
iOS. See the API availability guide for iOS for
details.
...
`AppleFrameworkLoader` that is able to read and
...
`ModuleSpec` for the loaded module will report the
...
7. Add a build step that processes the Python standard library, and your own
   ...
   install_pythonPython.xcframeworkapp
   ```
...
* If you’re using a separate folder for third-party packages, ensure that
  ...
  as part of the `PYTHONPATH` configuration in step 8.
* If any of the folders that contain third-party packages will contain `.pth`
  ...
  `site.addsitedir()`), rather than adding to `PYTHONPATH` or
  `sys.path` directly.

### 7.2.3. Testing a Python package

The CPython source tree contains a testbed project that
